...
To show 
  ...
  i) P ⇓ₛ b ∨
    =>h3
    ia) (b=db => db ∈ π) /\
      by h1 WP↓ ⇓ᵢ db, to show db ∈ π, OK

    ib) (b=ub(t) => ∀t'. t'≥t /\ ∃WP'. WP' ⇓ₛ db(t') => db(t') ∈ π)
      by h1 WP↓ ⇓ᵢ ub(t), to show ∀t'. t'≥t /\ ∃wp'. wp' ⇓ᵢ db(t') => db(t') ∈ π)
      Given t' t'≥t /\ ∃wp'. wp' ⇓ᵢ db(t'), to show db(t') ∈ π
      by VDEF on wp' and db(t'), ∃ WP'. WP' ⇓ₛ db(t')
      then db(t') ∈ π

  ii) (P ⇓ₛ ub(prefix(b)) ∧ ∃ P'. P' ⇓ₛ b)
