...
PCCR: P↓ ⇓ₜ{C} b => P ⇓ₛ{C} b \/ P ⇓ₛ{C} pref(b);UB(P)                   Partial Compile Correctness Backward and Refined
PCCRD: P↓ ⇓ₜ{C} b => P ⇓ₛ{C} b \/ (P ⇓ₛ{C} pref(b);UB(P) /\ ∃P'. P' ⇓ₛ{C} b)  Partial Compile Correctness Backward and Defined

We can go back to complete semantics obtaining the following
RC4:  ∀c FD. c[P↓] ⇓ᵢ b => ∃C. C[P] ⇓ₛ b \/ C[P] ⇓ₛ pref(b);UB(P)
RC4D: ∀c FD. c[P↓] ⇓ₜ b => ∃C. C[P] ⇓ₛ b \/ (C[P] ⇓ₛ pref(b);UB(P) /\ ∃P'. C[P'] ⇓ₛ b)
...
Explode UB / Suffix closed
SCL: π is refSig closed or ∀ ub(t,P) ∈π => refSig(ub(t,P)) ∈π
DEF is more precise than SCL, do we need it? In SCL π contains more stuff, even stuff that our semantics cannot generate, even if this is not nice, it doesn't bother the proof of preservation.
...
TODO understand relation between DEF and EUB
think about them in WP case too
TODO how do we prove this stuff?
HR looks like a DEC and DEF at the same level
TODO MD: ∀Cᵢ∈Cₛ.RC4(Cᵢ)
* Robust Execution
