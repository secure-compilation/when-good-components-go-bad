...
| Wrong : trace -> Component.id -> string -> ExecutionError -> execution_state.
...
Instance exec_monad : Monad (@execution_state)
  := {
      ...
      bind := fun {A B:Type} (x : @execution_state A) (f : A -> @execution_state B) =>
                match x with
                | Running y => f y
                | Wrong tr cid m err => Wrong tr cid m err
                | Halted tr => Halted tr
                | OutOfFuel y => f y
                end
    }.
...
Definition lift {A} (x: option A) (cid : Component.id) (msg : string) (err : ExecutionError) :=
  match x with
  | None  => (@Wrong A E0 cid msg err)
  | Some v => (@Running A v)
  end.

Definition fail {A} (cid : Component.id) (msg : string) (err : ExecutionError) :=
  (@Wrong A E0 cid msg err).
...
Definition eval_step (G: global_env) (s: CS.state)  : (@execution_state t) :=
  ...
  do C_procs <- lift (getm (genv_procedures G) (Pointer.component pc) )
    ...
    | None => fail (Pointer.component pc) "Missing block"%string (MissingBlock pc)
    | Some P_code =>
      if ((Pointer.offset pc) <? 0)%Z then
        fail (Pointer.component pc) "Negative offset"%string (NegativePointerOffset pc)
      else
        match nth_error P_code (Z.to_nat (Pointer.offset pc)) with
        | None => fail (Pointer.component pc) "Offset too big"%string (OffsetTooBig pc)
        | Some instr =>
          ...
          | ILoad r1 r2 =>
            ...
            | Ptr ptr =>
              if Component.eqb (Pointer.component ptr) (Pointer.component pc) then
                do v <- lift (Memory.load mem ptr)
                   (Pointer.component pc)
                   "Memory load error" (MemoryError ptr pc);
                  let regs' := Intermediate.Register.set r2 v regs in
                  ret (E0, (gps, mem, regs', Pointer.inc pc))
              else
                fail (Pointer.component pc) "Load outside component"%string LoadOutsideComponent
            | _ => fail (Pointer.component pc)
                        "Not a pointer value in address register"%string LoadNotAddressInReg
            end
          | IStore r1 r2 =>
            match Intermediate.Register.get r1 regs with
            | Ptr ptr =>
              if Component.eqb (Pointer.component ptr) (Pointer.component pc) then
                do mem' <- lift
                   (Memory.store mem ptr (Intermediate.Register.get r2 regs))
                   (Pointer.component pc)
                   "Memory store error"%string (StoreMemoryError ptr pc);
                  ret (E0, (gps, mem', regs, Pointer.inc pc))
              else
                fail (Pointer.component pc)
                     "Store outside component"%string StoreOutsideComponent
            | _ => fail (Pointer.component pc)
                        "Not a pointer value in address register"%string StoreNotAddressInReg
            end
          | IJal l =>
          ...
          | IJump r =>
            ...
            | Ptr pc' =>
              if Component.eqb (Pointer.component pc') (Pointer.component pc) then
                ...
                fail (Pointer.component pc)
                     "Jump outside component"%string JumpOutsideComponent
            | _ => fail (Pointer.component pc)
                        "Not a pointer value in address register"%string JumpNotAddressInReg
            end
          | IBnz r l =>
            match Intermediate.Register.get r regs with
            | Int 0 =>
            ...
            | Int val =>
              do pc' <- lift (find_label_in_procedure G pc l)
                ...
                ret (E0, (gps, mem, regs, pc'))
            | _ => fail (Pointer.component pc)
                        "Bnz::Not int"%string NotIntInReg
            end
          | IAlloc rptr rsize =>
            ...
            | Int size =>
              if (size <=? 0)%Z then
                fail (Pointer.component pc)
                     "Negative block size"%string AllocNegativeBlockSize
              else
                ...
                ret (E0, (gps, mem', regs', Pointer.inc pc))
            | _ => fail (Pointer.component pc)
                        "Alloc::Not int"%string NotIntInReg
            end
          | ICall C' P =>
                ...
                do b <- lift (Intermediate.EntryPoint.get C' P (genv_entrypoints G))
                  ...
                  | Int rcomval =>
                    ...
                    let res : t := (tr, (Pointer.inc pc :: gps,
                                         ...
                                         pc')) in
                    ret res
                  | Ptr _ => fail (Pointer.component pc)
                                  "Call::Ptr in register R_COM"%string NoInfo
                  | Undef => fail (Pointer.component pc)
                                  "Call::Undef in register R_COM"%string NoInfo
                  end
              else
                fail (Pointer.component pc)
                     "Call::procedure not imported"%string InvalidEnv
            else
              fail (Pointer.component pc)
                   "Call::same component"%string InvalidEnv
          | IReturn =>
            match gps with
            | pc' :: gps' =>
              if negb (Component.eqb (Pointer.component pc) (Pointer.component pc')) then
                ...
                | Ptr _ => fail (Pointer.component pc)
                             "Return::Ptr in register R_COM"%string NoInfo
                | Undef => fail (Pointer.component pc)
                                "Return::Undef in register R_COM"%string NoInfo
                end
              else
                fail (Pointer.component pc)
                     "Return::same component"%string InvalidEnv
            | _ => fail (Pointer.component pc) "Empty Stack"%string InvalidEnv
            end
          | IHalt =>  Halted E0
          end
        end
...
Fixpoint execN (n: nat) (G: global_env) (tr:trace) (st: CS.state) : execution_state :=
  ...
  | S n' =>
    ...
    | Wrong _ cid  msg err => Wrong tr cid msg err
