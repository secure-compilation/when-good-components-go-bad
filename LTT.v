...
Reserved Notation "Is ; E |-LTT s1 '=>[' t ']' s2" (at level 40).
...
Inductive step (Is : Program.interface) (E : EntryPoint.data)
  ...
  | Nop: forall mem C pc d regs,
      ...
      Is;E |-LTT (C,d,mem,regs,pc) =>[E0] (C,d,mem,regs,pc+1)
  ...
  | Return: forall mem C pc pc' d' regs d C',
      ...
      Is;E |-LTT (C,d,mem,regs,pc) =>[t] (C',d',mem,regs,pc')

  where "Is ; E |-LTT s1 '=>[' t ']' s2" := (step Is E s1 t s2).

Lemma epsilon_step_preserves_component_and_stack:
  forall Is E C d mem regs pc C' d' mem' regs' pc',
    Is; E |-LTT (C,d,mem,regs,pc) =>[E0] (C',d',mem',regs',pc') ->
    C' = C /\ d' = d.
Proof.
  intros Is E C d mem regs pc C' d' mem' regs' pc' Hstep.
...
Lemma epsilon_step_weakening:
  forall Is E C d1 mem mem' cmem cmem' regs regs' pc pc',
    M.MapsTo C cmem  mem ->
    M.MapsTo C cmem' mem' ->
    Is; E |-LTT (C,d1,mem,regs,pc) =>[E0] (C,d1,mem',regs',pc') ->
  forall E' d2 wmem,
    M.MapsTo C cmem wmem ->
    exists wmem',
      M.MapsTo C cmem' wmem' ->
      Is; E' |-LTT (C,d2,wmem,regs,pc) =>[E0] (C,d2,wmem',regs',pc').
Proof.
...
Theorem LTT_determinism :
  forall Is E s t s1 s2,
    Is;E |-LTT s =>[t] s1 ->
    Is;E |-LTT s =>[t] s2 ->
    s1 = s2.
Proof.
  intros Is E s t s1 s2 E1 E2.
  inversion E1 as [ mem C pc d regs Hexec |
                    mem C pc d regs w0 w1 w2 Hexec |
                    mem C pc d regs w0 w1 w2 Hexec |
                    mem C pc d regs w0 w1 w2 w3 w4 w5 w6 Hexec |
                    mem C pc d regs w0 w1 w2 w3 w4 Hexec |
                    mem C pc d regs w0 w1 w2 w3 w4 Hexec |
                    mem C pc d regs w0 w1 w2 Hexec |
                    mem C pc d regs w0 w1 Hexec |
                    mem C pc d regs w0 w1 Hexec |
                    mem C pc d regs w0 w1 Hexec |
                    mem C pc d regs w0 w1 w2 Hexec |
                    mem C pc d regs w0 w1 w2 Hexec ];
    subst;
    inversion E2 as [ mem'' C'' pc'' d'' regs'' Hexec' |
                      mem'' C'' pc'' d'' regs'' z0 z1 z2 Hexec' |
                      mem'' C'' pc'' d'' regs'' z0 z1 z2 Hexec' |
                      mem'' C'' pc'' d'' regs'' z0 z1 z2 z3 z4 z5 z6 Hexec' |
                      mem'' C'' pc'' d'' regs'' z0 z1 z2 z3 z4 Hexec' |
                      mem'' C'' pc'' d'' regs'' z0 z1 z2 z3 z4 Hexec' |
                      mem'' C'' pc'' d'' regs'' z0 z1 z2 Hexec' |
                      mem'' C'' pc'' d'' regs'' z0 z1 Hexec' |
                      mem'' C'' pc'' d'' regs'' z0 z1 Hexec' |
                      mem'' C'' pc'' d'' regs'' z0 z1 Hexec' |
                      mem'' C'' pc'' d'' regs'' z0 z1 z2 Hexec' |
                      mem'' C'' pc'' d'' regs'' z0 z1 z2 Hexec' ];
