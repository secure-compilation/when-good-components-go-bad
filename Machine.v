...
Module AbstractMachine.
  ...
  Definition call_exists
             (Is : Program.interface)
             (C : Component.id) (P : Procedure.id) :=
    forall comp_interface,
      nth C Is None = Some comp_interface ->
      C < Component.export comp_interface.

  Definition call_permitted
             (Is : Program.interface)
             (C C': Component.id) (P : Procedure.id) :=
    forall comp_interface,
      nth C Is None = Some comp_interface ->
      In (C',P) (Component.import comp_interface).
