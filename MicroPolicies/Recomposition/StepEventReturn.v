Require Import Common.Definitions.
Require Import Common.Values.
Require Import Common.Util.
Require Import Common.Memory.
Require Import Common.Linking.
Require Import Common.CompCertExtensions.
Require Import CompCert.Events.
Require Import CompCert.Smallstep.
Require Import CompCert.Behaviors.
Require Import Common.SmallstepUB.

From mathcomp Require Import ssreflect ssrfun ssrbool eqtype ssrnat seq.
From CoqUtils Require Import hseq word.
From extructures Require Import fmap.

Set Implicit Arguments.
Unset Strict Implicit.
Unset Printing Implicit Defensive.

Set Bullet Behavior "Strict Subproofs".

Require Import MicroPolicies.Int32.
Require Import MicroPolicies.Symbolic.
Require Import MicroPolicies.Instance.
Require Import MicroPolicies.LRC.
Require Import MicroPolicies.Merged.
Require Import Coq.micromega.Lia.
Require Import Coq.Program.Equality.

Require Import MicroPolicies.Utils.
Import DoNotation.
Import Types.

Require Import MicroPolicies.Recomposition.Definitions.
Require Import MicroPolicies.Recomposition.PreservationLemmas.

Module StepEventReturn (S: RecompositionContext).

  Module Defs := RecompositionDefinitions S.
  Module Pres := Preservation S.
  Include Pres.

  Lemma step_event_return:
  forall s1 comp z comp' s1', Step sem s1 ((ERet comp z comp') :: nil) s1' ->
  forall s2 s2',   Step sem' s2 ((ERet comp z comp') :: nil) s2' ->
  forall s3 M, match_states M s1 s2 s3    ->
  exists s3' M', Plus sem'' s3 ((ERet comp z comp') :: nil) s3' /\ (* using Plus here because we know if an event is emitted then we've done at least one step *)
              match_states M' s1' s2' s3'.
  Proof.
    import_context.
    intros s1 comp z comp' s1' step_s1 s2 s2' step_s2 s3 M match_st.
    remember (id s3) as s3'; simpl in Heqs3'; destruct s3' as [mem3 regs3 [pc3_val pc3_tag] internal3 cn3].
    rewrite Heqs3' in match_st; rewrite Heqs3'. pose proof (esym Heqs3') as eq_s3. clear Heqs3'.
    inversion step_s2;
      unfold next_state_updates, next_state_updates_and_pc, next_state, transfer, instr_rules, LRC.instr_rules in *.
    all: unfold_all; try unfold_match; unfold_all.
    all: try (clear -Heqa2; repeat unfold_match' Heqa2; done); simpl in *. (* eliminates many goals *)
    (* gets rid of cases with no events *)
    1-6, 8: (exfalso; clear -NEXT Heqa0 Heqa4 Heqa7; repeat (unfold_all || unfold_match); done).
    3: (exfalso; clear -CALL; unfold run_syscall in *; repeat (unfold_all || unfold_match); done).
    all: (assert (tpc1 = tpc0) by (clear -Heqa2; unfold_match' Heqa2); subst tpc1).
    all: (assert (ts0 = (ts (mvec None)) ) by
             (unfold mvec in *; simpl; clear -Heqa2; unfold_match' Heqa2; try (inversion Heqa2; subst b); eapply ivec_eq_inv in Heqa2;
              destruct Heqa2 as [_ _ _ H]; eapply Classical_Prop.EqdepTheory.inj_pair2 in H; auto); subst ts0).
    all: (unfold mvec in *; assert (ti1 = ti0) by
                (clear -Heqa2; unfold_match' Heqa2; try (inversion Heqa2; subst b); eapply ivec_eq_inv in Heqa2;
                 destruct Heqa2 as [_ _ H]; eapply Classical_Prop.EqdepTheory.inj_pair2 in H; auto); subst ti1).
    all: unfold check_belong, belong, reg_clear_list in *.
    all: repeat (unfold_all || unfold_match).
    all: destruct tpc0; repeat (unfold_all || unfold_match); simpl in *.
    all: convert_eq_op.
    all: unfold is_jump, check_ret in *; try (repeat unfold_match; subst t1).
    all: unfold hshead in *; simpl in *.
    all: (inversion Heqa0); revert ST eq_s3; subst; intros ST eq_s3. (* trick to keep an equality on s1 and s3 *)
    all: simpl in *.
    1: rename i1 into comp; rename color0 into comp'.
    (* tidying up the hypothesis *)
    all: simpl in *.
    all: repeat
           (match goal with
              H: Some _ = getm (setm _ _ _) _ |- _ =>
                repeat (rewrite setmE in H; unfold as_word in H; simpl in H)
            end).
    all: repeat
           (match goal with
              H: Some _ = match (eq_op (_ (_ ((_ (_ ?a _)) _ _))) _) with _ => _ end,
                H' : Some _ = getm _ (as_word (_ ?a))
              |- _ => idtac a; unfold as_word in H'; rewrite ST in H; simpl in H', H; rewrite <- H' in H
            end).
    all: simpl in *.
    all: repeat (match goal with H: ?a = ?a |- _ => clear H end).
    (* we branch on which side is strongly related to s3 *)
    all: destruct match_st as [M ? ? ? common strong weak |M ? ? ? common weak strong];
      (* in each case, we keep only one step from s1 (the most useful for the case being treated) *)
      [match (type of step_s1) with
         (_ _ _ _ _ _ ?ev _) =>
           remember ev as t;
           destruct step_s1 as [s1 ? s1' step_s1 allowed | s1 ? s1' step_s1 _];
           [exfalso;
            destruct strong as [? ? ? pc_s1_s3 color_eq side_eq s_mem_cor s'_mem_cor s_reg_cor s'_reg_cor mem_match reg_match];
            eapply Machine.Intermediate.fdisjoint_partition_notinboth;
            [inversion Hmergeable_ifaces as [[_ fdisj] _]; exact fdisj | exact allowed | unfold side_of in *; unfold_match' side_eq]
           | ]; subst t
       end
      |match (type of step_s1) with
         (_ _ _ _ _ _ ?ev _) =>
           assert (step_2: step2 tt s1 ev s1');
           [remember ev as t; destruct step_s1 as [s1 ? s1' step_s1 allowed | s1 ? s1' _ step_s1]; subst t; exact step_s1
           | clear step_s1; rename step_2 into step_s1]
       end].
    repeat
      match goal with
        H: Some ?a = match ?cond with true => Some ?b | false => Some ?c end |- _ =>
          assert (a = if cond then b else c); [destruct cond; inversion H; try congruence| clear H]
      end.
    all: inversion step_s1;
      unfold next_state_updates, next_state_do_updates, next_state_updates_and_pc,
      next_state, transfer, instr_rules, LRC.transfer, LRC.instr_rules in *.
    all: unfold_all; try unfold_match; unfold_all.
    all: try (exfalso; clear -CALL; unfold run_syscall in *; repeat (unfold_all || unfold_match)). (* eliminate alloc cases *)
    all: match goal with
         | H: @eq (ivec lrc_tags) _ _ |- _ => rename H into ivec_eq
         end.
    all: try (exfalso; unfold mvec0 in ivec_eq; clear- ivec_eq; repeat unfold_match' ivec_eq; done). (* eliminate many uncoherent cases *)
    all: (assert (tpc1 = tpc0) by (clear -ivec_eq; unfold_match' ivec_eq); subst tpc1).
    all: destruct tpc0.
    all: assert (eqo1: OP o1 = op (mvec0 None)) by (try subst o1; unfold mvec0 in *; clear -ivec_eq; unfold_match' ivec_eq).
    all: unfold mvec0 in eqo1; simpl in eqo1; inversion eqo1; subst o1.
    all: (assert (ts0eq: ts0 = (ts (mvec0 None)) ) by
           (unfold mvec0 in *; simpl; clear -ivec_eq; unfold_match' ivec_eq; try (inversion ivec_eq; subst b);
            eapply ivec_eq_inv in ivec_eq; destruct ivec_eq as [_ _ _ H]; eapply Classical_Prop.EqdepTheory.inj_pair2 in H; auto);
          unfold mvec0 in ts0eq; simpl in ts0eq; subst ts0).
    all: (unfold mvec0 in *; assert (ti1 = ti0) by
                (clear -ivec_eq; unfold_match' ivec_eq; try (inversion ivec_eq; subst b); eapply ivec_eq_inv in ivec_eq;
                 destruct ivec_eq as [_ _ H]; eapply Classical_Prop.EqdepTheory.inj_pair2 in H; auto); subst ti1).
    all: unfold check_belong, belong, reg_clear_list in *.
    all: repeat
           let pl := fresh "pl" in
           let pr := fresh "pr" in
           match goal with
           | p : _ * option event |- _ =>
               match goal with
               | H : Some p = _ |- _ =>
                   assert (tmp: p.2 = None) by (clear -H; repeat (unfold_all || unfold_match));
                   destruct p as [pl pr]; inversion tmp; clear tmp; simpl in *; try subst pl; try subst pr
               end
           end.
    all: try simplify_some.
    all: try (exfalso; clear -NEXT; repeat (unfold_all || unfold_match); done). (* eliminate all remaining cases with no events *)
    all: unfold reg_clear_list_aux in CLEAR; unfold_all.
    2, 4: exfalso;
    repeat ( match goal with | p : _ * option event |- _ => destruct p end); simpl in *;
    repeat (unfold_all || unfold_match). (* eliminate cases with the wrong events *)
    all: repeat
           match goal with
             H: Some ?a = match ?cond with true => Some ?b | false => Some ?c end |- _ =>
               assert (a = if cond then b else c); [destruct cond; inversion H; try congruence| clear H]
           end.
    all: revert Heqa23; repeat (unfold_all || unfold_match).
    all: intro a21_eq.
    all: subst color; try subst color0; try subst color1; try subst i3; destruct e0, e; simpl in *; subst rcom_value0.
    all: unfold is_jump, check_ret, build_tpc in *; try (revert a21_eq; repeat unfold_match; subst t1; intro a21_eq).
    all: convert_eq_op.
    all: unfold_match' ivec_eq; inversion ivec_eq as [tag_mem_w0].
    all: try (match goal with
                H : @eq (ovec lrc_tags _) _ _ |- _ => inversion H; clear H
              end).
    all: revert ST ST0 eq_s3; subst; intros ST ST0 eq_s3; simpl in *. (* trick to keep an equality on s1 and s3 *)
    1-2: rename i1 into comp.
    all: inversion strong as [? ? ? pc_s'_s3 color_eq side_eq s_mem_cor s'_mem_cor entry_off s_reg_cor s'_reg_cor mem_match reg_match].
    all: inversion common as [? ? ? ? ? ? tag_pc1 tag_pc2 tag_pc3 wfst reg_domm1 reg_domm2 reg_domm3
                          [mem_pref_cond_s1 [mem_pref_cond_s1' [code_pref_cond_s1 [alloc_mem_s1 [bnz_s1 [end_s1 [col_mem1 entry_code1]]]]]]]
                          [mem_pref_cond_s2 [mem_pref_cond_s2' [code_pref_cond_s2 [alloc_mem_s2 [bnz_s2 [end_s2 [col_mem2 entry_code2]]]]]]]
                          [mem_pref_cond_s3 [mem_pref_cond_s3' [code_pref_cond_s3 [alloc_mem_s3 [bnz_s3 [end_s3 [col_mem3 entry_code3]]]]]]]
                          capa_cor1 capa_cor2 capa_cor3 code_left code_right].
    all: subst s s' s0 s4 s5 m m0.
    (* We now need to branch on which state is strongly related to s3 next (i.e. on what is comp', the color of the next compartment) *)
    all: ((match goal with
           H: Some ?a = getm (mem ?s) ?w, col_mem: color_in_memory (mem ?s) |- _ =>
             destruct (col_mem _ _ (esym  H)) as [ip_comp' | ic_comp']; destruct a; simpl in *;
             [setoid_rewrite <- Extra.In_in in ip_comp'; rewrite <- tag_mem_w0 in ip_comp'; simpl in ip_comp'
             |setoid_rewrite <- Extra.In_in in ic_comp'; rewrite <- tag_mem_w0 in ic_comp'; simpl in ic_comp']
         end) || exfalso).
    all: rewrite ST in tag_pc2; inversion tag_pc2; subst n0 c0; clear tag_pc2.
    all: repeat match goal with
             |- context[vala (match ?cond with true => ?a | false => ?b end)] =>
               assert (tmp: vala (match cond with true => a | false => b end) = (match cond with true => vala a | false => vala b end));
               [ clear; destruct cond; simpl; done | rewrite tmp; clear tmp]
           end; simpl.
    all: ((destruct pc_s'_s3 as [? ? ? eq_none|s1 s3 ? ? _ eq_off pc_s'_s3]
          || destruct pc_s'_s3 as [? ? ? eq_none|s2 s3 ? ? _ eq_off pc_s'_s3]);
          [exfalso; subst; simpl in *; (rewrite eq_none in PC) || (rewrite eq_none in PC0); done |]).
    (* Steps needed in each cases: *)
    (* 1/ deduce beforehand the equalities on s3 needed to proceed, *)
    (* 2/ unfold to show that a step can be done from s3, *)
    (* 3/ show that match_state hold for the new states. *)
    + (* return from comp in P to comp' in P *)
      repeat
        (match goal with
           H : Some ?a = getm reg0 _ |- _ => deduce_equality (esym H); pose proof (esym H); clear H
         end).
      deduce_equality RW0.
      deduce_equality RA0.
      subst s1. unfold side_of in *. unfold_match' side_eq. simpl in *.
      deduce_equality PC0.
      assert (Meq: exists top M', M = top :: M').
      { destruct vt_match10 as [ret_eq stack_prop]. unfold color_of in stack_prop.
        subst t10. simpl in stack_prop. destruct stack_prop as [top [col top_in]].
        destruct M; eauto. inversion top_in. }
      destruct Meq as [top [M' M_eq]]. destruct a41 as [xt xv].
      destruct vt_match10 as [ret_eq stack_prop]. unfold color_of in stack_prop.
      subst t10. simpl in stack_prop. destruct stack_prop as [s2val [comp'' triple_in]].
      match (type of triple_in) with (_ ?t _) => assert (top = t); [|subst top] end.
      { inv tag_pc1. simpl in *. subst.
        clear- wfst triple_in. destruct triple_in as [?| is_in]; auto. exfalso.
        remember (Nat.pred n) as n'. assert (n'_ineq: S n' >= n) by (subst; clear; induction n; eauto).
        clear Heqn'.
        revert is_in. revert n'_ineq. revert w0 s2val v11 comp'' n'.
        inversion wfst; subst; clear - WF_ST; simpl; unfold Init.Nat.pred, "+", addn_rec; rewrite Nat.add_1_r.
        all: induction WF_ST; intros a v b col n' n'_ineq is_in; inversion is_in.
        all: try (inversion H; subst n; (assert false; [|done]); rewrite <- (ltn_subrR 0 (S n')); unfold subn, subn_rec, Nat.sub;
                  unfold addn, addn_rec in n'_ineq; rewrite Nat.add_1_r in n'_ineq; done).
        all: eapply (IHWF_ST _ _ _ _ _ _ H). }
      assert (comp'' = comp'); [|subst comp''].
      { clear - wfst M_eq Heqa7 tag_mem_w0. inversion wfst; subst M; inv H0;
        unfold points_to_comp_code' in PTS_CODE1; oapp_False; simpl in *; rewrite <- Heqa7 in HeqH; simplify_some;
        destruct PTS_CODE1 as [col ?]; simpl in col; auto. }
      inversion wfst; subst M; try (inv H0; done); simpl in *.
      2: { exfalso. inv H14. clear - SIDE ip_comp' Hmergeable_ifaces.
           eapply (@Machine.Intermediate.fdisjoint_partition_notinboth _ (domm ip) (domm ic)); eauto.
           inversion Hmergeable_ifaces as [[_ fdisj] _]; eauto. }
      inversion H14. subst v14 n n0 s2val v16 C st. clear H20 H14.
      unfold points_to_comp_code in PTS_CODE3. oapp_False.
      eexists. exists M'.
      split.
      * eapply (plus_left _ [:: ERet comp rcom_value1 comp'] ). try eapply step_jump; eauto.
        -- rewrite eq_s3 in vt_eq12. exact vt_eq12.
        -- inversion vt_match12; subst; revert H11; try decode_instr_eq.
        -- rewrite eq_s3 in vt_eq10. exact vt_eq10.
        -- rewrite eq_s3 in vt_eq11. exact vt_eq11.
        -- unfold reg_clear_list, reg_list. simpl. unfold as_word. subst. simpl.
           repeat
             (match goal with
              | H: _ ?w = _  |- context[_ ?w] => rewrite H; simpl
              end). trivial.
        -- unfold next_state_updates, next_state_updates_and_pc, next_state, transfer, instr_rules, LRC.instr_rules in *.
           unfold evi in *. simpl. unfold_bind.
           deduce_reg reg_match. subst v11. simpl in *.
           rewrite <- HeqH11. simpl. rewrite eq_s3 in tag_pc3. simpl in tag_pc3.
           rewrite tag_pc3. rewrite eq_refl. simpl.
           destruct PTS_CODE3 as [col_a10 code_a10]. destruct a10 as [va10 ta10]. destruct ta10.
           simpl in col_a10. rewrite col_a10. simpl. rewrite <- Heqa10. simpl.
           destruct a23 as [? ta]. unfold is_other in *. unfold_match' Heqa51. simpl in *.
           subst ta. inversion vt_match9 as [? _]. subst t9. simpl.
           unfold "+", addn_rec. rewrite Nat.add_1_r. simpl. rewrite eq_refl. simpl.
           unfold updm. unfold as_word. simpl in *.
           (*automatically rewrite regs3 registers*)
           repeat
             (repeat
                (match goal with
                 | H: getm ?r ?w = _  |- context[getm ?r ?w] => rewrite H; simpl
                 end);
              (*automatically transforms setm in if (_ == _) then _ else _*)
              repeat rewrite setmE; simpl;
              (*destruct the (_ == _) condition *)
              try
                (let cond := fresh "cond" in
                 match goal with
                 | |- context[@eq_op ?t ?a ?r] =>
                     remember (@eq_op t a r) as cond; destruct cond;
                     [convert_eq_op; simpl in *|]; simpl
                 end)).
           all: inversion Heqa0; subst rcom_value1; rewrite H10 in Heqa5; simplify_some; destruct x; destruct H11; subst; simpl.
           13: reflexivity. (* most general case, to be treated first *)
           all: unfold as_word in *; simpl in *.
           all: (match (type of vt_eq10) with
                 | ?ls = ?rs =>
                     revert vt_eq10;
                     (match goal with
                        Heq: ls = Some (?a) |- _ =>
                          intro vt_eq10; rewrite Heq in vt_eq10; simplify_some; subst; simpl
                      end)
                 end).
           all: repeat (rewrite setmxx || (rewrite (setmC _ ((addw _ _)@_)); [|done]) ).
           all: trivial.
        -- eapply star_refl.
        -- reflexivity.
      * inversion tag_pc1. rewrite eq_s3 in tag_pc3. simpl in tag_pc3. rewrite <- H14 in tag_pc3. subst pc3_tag.
        clear tag_pc1. subst. simpl in *.
        repeat rewrite <- H14.
        eapply match_states_left; econstructor; simpl; unfold build_tpc; try trivial.
        -- unfold register_domm. repeat rewrite domm_set. simpl.
           unfold register_domm, reg_field_size, mword, FSet.fsval in reg_domm1. simpl in reg_domm1.
           unfold mword, word_size. simpl.
           (* clear - reg_domm1 RW0. admit. (* exstructure *) *)
           clear -reg_domm1 RW0.
           rewrite //= reg_domm1 //=.
           repeat match goal with
             | |- context [as_word (ssrint.Posz ?n)] =>
                 replace (@as_word 5 (ssrint.Posz n)) with
                 (@word_of_nat 5 n) by reflexivity
           end.
           rewrite <- eq_fset.
           intros r. rewrite in_fset.
           match goal with
           | |- (r \in (word_of_nat _ |: _)%fset) = (r \in _) =>
               rewrite in_fset in_cons
           end;
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=| clear eqr]
           end.
           repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
           rewrite in_fset in_cons.
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
           end.
           ++ by rewrite -in_fset -reg_domm1 mem_domm RW0.
           ++ repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
              now rewrite in_fset.
        -- unfold register_domm. repeat rewrite domm_set. simpl.
           unfold register_domm, reg_field_size, mword, FSet.fsval in reg_domm2. simpl in reg_domm2.
           unfold mword, word_size. simpl.
           clear -reg_domm2 RW.
           rewrite //= reg_domm2 //=.
           repeat match goal with
             | |- context [as_word (ssrint.Posz ?n)] =>
                 replace (@as_word 5 (ssrint.Posz n)) with
                 (@word_of_nat 5 n) by reflexivity
           end.
           rewrite <- eq_fset. rename r into r0.
           intros r. rewrite in_fset.
           match goal with
           | |- (r \in (word_of_nat _ |: _)%fset) = (r \in _) =>
               rewrite in_fset in_cons
           end;
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=| clear eqr]
           end.
           repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
           rewrite in_fset in_cons.
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
           end.
           ++ by rewrite -in_fset -reg_domm2 mem_domm RW.
           ++ repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
              now rewrite in_fset.
        -- unfold register_domm. repeat rewrite domm_set. simpl.
           unfold register_domm, reg_field_size, mword, FSet.fsval in reg_domm3. simpl in reg_domm3.
           unfold mword, word_size. simpl.
           clear - reg_domm3 vt_eq10.
           rewrite //= reg_domm3 //=.
           repeat match goal with
             | |- context [as_word (ssrint.Posz ?n)] =>
                 replace (@as_word 5 (ssrint.Posz n)) with
                 (@word_of_nat 5 n) by reflexivity
           end.
           unfold word_of_nat. unfold as_word. simpl.
           rewrite <- eq_fset.
           intros r. rewrite in_fset.
           match goal with
           | |- (r \in (Word _ |: _)%fset) = (r \in _) =>
               rewrite in_fset in_cons
           end;
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=| clear eqr]
           end.
           repeat (match goal with
                   | |- (r \in FSet.fsval (Word _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
           rewrite in_fset in_cons.
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
           end.
           ++ by rewrite -in_fset -reg_domm3 mem_domm vt_eq10.
           ++ repeat (match goal with
                   | |- (r \in FSet.fsval (Word _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
              now rewrite in_fset.
        -- split; [|split; [|split; [|split; [|split; [|split; [| split]]]]]]; trivial.
        -- split; [|split; [|split; [|split; [|split; [|split; [| split]]]]]]; trivial.
        -- split; [|split; [|split; [|split; [|split; [|split; [| split]]]]]]; trivial.
        -- intros v' n rw rweq. simpl in *. destruct rw as [r' | w'].
           { exfalso. repeat rewrite setmE in rweq. repeat unfold_match' rweq.
             (* move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in. *)
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               now rewrite -in_fset -reg_domm1 mem_domm rweq.
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in. }
           pose proof (capa_cor1 v' n (inr w') rweq) as [is_in unicity]. simpl in unicity. split.
           { unfold in_stack in *. destruct is_in as [sv1 [sv2 [col [|is_in]]]]; [|eauto].
             exfalso. inversion H11. subst n v' sv1 sv2 col. eapply (unicity (inl _)). exact RW0. }
           intros rw v''. destruct rw as [r' | w'']; simpl.
           { intro rweq'. exfalso. repeat rewrite setmE in rweq'. repeat unfold_match' rweq'.
             (* assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by admit. *)
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               now rewrite -in_fset -reg_domm1 mem_domm rweq'.
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in.  }
           intros [t' [v''eq t'eq]]. eapply (unicity (inr _)); eauto.
        -- intros v' n rw rweq. simpl in *. destruct rw as [r' | w'].
           { exfalso. repeat rewrite setmE in rweq. repeat unfold_match' rweq.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               now rewrite -in_fset -reg_domm2 mem_domm rweq.
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in. }
           pose proof (capa_cor2 v' n (inr w') rweq) as [is_in unicity]. simpl in unicity. split.
           { unfold in_stack in *. destruct is_in as [sv1 [sv2 [col [|is_in]]]]; [|eauto].
             exfalso. inversion H11. subst n v' sv1 sv2 col. eapply (unicity (inl _)). rewrite H14. exact RW. }
           intros rw v''. destruct rw as [r' | w'']; simpl.
           { intro rweq'. exfalso. repeat rewrite setmE in rweq'. repeat unfold_match' rweq'.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               now rewrite -in_fset -reg_domm2 mem_domm rweq'.
             (* assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
(rewrite <- reg_domm2; rewrite mem_domm; rewrite rweq'; trivial). *)
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in.  }
           intros [t' [v''eq t'eq]]. eapply (unicity (inr _)); eauto.
        -- intros v' n rw rweq. simpl in *. destruct rw as [r' | w'].
           { exfalso. repeat rewrite setmE in rweq. repeat unfold_match' rweq.
             repeat simplify_some. inversion Heqa51.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               now rewrite -in_fset -reg_domm3 mem_domm rweq.
             (* assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by admit. *)
             (* assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
(rewrite <- reg_domm3; rewrite mem_domm ; rewrite rweq; trivial). *)
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold as_word in r'_in. simpl in r'_in.
             unfold word_of_nat, as_word in r'_in. simpl in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in. }
           pose proof (capa_cor3 v' n (inr w') rweq) as [is_in unicity]. simpl in unicity. split.
           { unfold in_stack in *. destruct is_in as [sv1 [sv2 [col [|is_in]]]]; [|eauto].
             exfalso. inversion H11. subst n v' sv1 sv2 col. eapply (unicity (inl _)). exact vt_eq10. }
           intros rw v''. destruct rw as [r' | w'']; simpl.
           { intro rweq'. exfalso. repeat rewrite setmE in rweq'. repeat unfold_match' rweq'.
             repeat simplify_some. inversion Heqa51.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               now rewrite -in_fset -reg_domm3 mem_domm rweq'.
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. simpl in r'_in. unfold word_of_nat, as_word in r'_in. simpl in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in.  }
           intros [t' [v''eq t'eq]]. eapply (unicity (inr _)); eauto.
        -- eapply same_pc_normal; simpl; eauto.
        -- unfold side_of. rewrite ip_comp'. trivial.
        -- unfold register_address_correctness. simpl.
           intros d r1 r_d_eq.
           unfold register_domm in *.
           assert (r_in : In r1 (domm reg0)).
           { clear - reg_domm1 r_d_eq RW0. rewrite reg_domm1.
             rewrite -Extra.In_in in_fset. simpl.
             repeat (rewrite setmE in r_d_eq; unfold_match' r_d_eq; [ convert_eq_op |]; simpl in r_d_eq).
             all: try (red; simpl; tauto).
             rewrite -in_fset -reg_domm1. setoid_rewrite <- (rwP dommP). eauto.
             rewrite -in_fset -reg_domm1. setoid_rewrite <- (rwP dommP). eauto. }
           rewrite reg_domm1 in r_in.
           move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
           repeat (destruct r_in as [? | r_in]; try subst r1;
                   repeat (rewrite setmE in r_d_eq; simpl in r_d_eq)).
           all: unfold_match' r_d_eq; simpl.
        -- unfold register_address_correctness. simpl.
           intros d r1 r_d_eq.
           unfold register_domm in *.
           assert (r_in : In r1 (domm regs3)).
           { clear - reg_domm3 r_d_eq vt_eq10. rewrite reg_domm3 -Extra.In_in in_fset; simpl.
             repeat (rewrite setmE in r_d_eq; unfold_match' r_d_eq; [ convert_eq_op |]; simpl in r_d_eq).
             all: try (red; simpl; tauto).
             rewrite -in_fset -reg_domm3. setoid_rewrite <- (rwP dommP). eauto.
             rewrite -in_fset -reg_domm3. setoid_rewrite <- (rwP dommP). eauto. }
           rewrite reg_domm3 in r_in.
           move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
           repeat (destruct r_in as [? | r_in]; try subst r1; repeat (rewrite setmE in r_d_eq; simpl in r_d_eq));
             simpl in r_d_eq; repeat simplify_some; simpl; trivial. unfold_match' r_d_eq.
           { destruct d; unfold is_other in *; unfold_match' Heqa51. }
           inversion r_in.
        -- unfold memory_match. simpl.
           intros w' d d_code d_ip. split; intro w_eq.
           { destruct (fst (mem_match w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H15; subst. destruct d''t; try exact H15. destruct H15 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H15. subst n vala1 vala0.
             match (type of d''eq) with _ = Some (?v) => assert (rel: is_relevant_comp Left (LRC.color (taga v))) by (simpl; auto) end.
             destruct (s_mem_cor _ _ rel d''eq) as [d' [d'eq d'code]]. simpl in d'eq.
             pose proof (capa_cor1 w0 (n1.-1) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. eauto.
             eapply (unicity (inl _)); eauto. }
           { destruct (snd (mem_match w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H15; subst. destruct d''t; try exact H15. destruct H15 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H15. subst n vala1 vala0.
             match (type of d''eq) with _ = Some (?v) => assert (rel: is_relevant_comp Left (LRC.color (taga v))) by (simpl; auto) end.
             subst. destruct (s'_mem_cor _ _ rel d''eq) as [d' [d'eq d'code]]. simpl in d'eq.
             pose proof (capa_cor3 (w0 + as_word off0)%w (n1.-1) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. eauto.
             eapply (unicity (inl _)); eauto. }
        -- unfold registers_match. simpl. intros w' d.
           split; intro w_eq.
           { repeat (rewrite setmE in w_eq; unfold_match' w_eq; [ convert_eq_op |]; simpl in w_eq).
             all: try simplify_some.
             all: repeat (rewrite setmE; simpl).
             1-11: eexists; split; [| reflexivity].
             1-11: try (split; trivial).
             { destruct d as [v_d t_d]. unfold is_other in *. destruct t_d; inversion Heqa51.
               split; trivial. rewrite setmE in Heqa41.
               unfold_match' Heqa41; simplify_some; trivial. simpl. convert_eq_op.
               rewrite setmE in Heqa40. unfold_match' Heqa40. rewrite <- Heqa40 in H10. simplify_some. trivial.
               rewrite setmE in Heqa41. unfold_match' Heqa41. rewrite <- Heqa41 in H10. simplify_some. trivial. }
             { simpl. unfold as_word. unfold ssrint.absz. simpl.
               repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end).
               simpl. subst pc3_val. simpl. eexists ((Types.vala a45)@Invalidated). eexists. split; auto.
               rewrite eq_refl. trivial. }
             { rewrite <- Heqa31. eexists (_ @ Invalidated). split; eauto. split; auto. }
             { assert (r_in : In w' (domm regs3)).
               { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). exists d. trivial. }
               exfalso. rewrite reg_domm3 in r_in. simpl in r_in. unfold word_of_nat, as_word in r_in. simpl in r_in.
               move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
               repeat (destruct r_in as [|r_in];
                       [subst;
                        (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
               inversion r_in. } }
           { repeat (rewrite setmE in w_eq; unfold_match' w_eq; [ convert_eq_op |]; simpl in w_eq).
             all: try simplify_some.
             all: repeat (rewrite setmE; simpl).
             1-10: eexists; split; [| reflexivity].
             1-10: try (split; trivial).
             { destruct a23 as [v_d t_d]. unfold is_other in *. destruct t_d; inversion Heqa51.
               eexists. split; trivial. rewrite setmE in Heqa41.
               unfold_match' Heqa41; simplify_some; trivial. simpl. convert_eq_op.
               rewrite setmE in Heqa40. unfold_match' Heqa40. rewrite <- Heqa40 in H10. simplify_some. split; trivial.
               rewrite setmE in Heqa41. unfold_match' Heqa41. rewrite <- Heqa41 in H10. simplify_some. split; trivial. }
             { simpl. unfold as_word, ssrint.absz in *. simpl in *.
               repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end).
               simpl. subst pc3_val. simpl. eexists ((Types.vala a45 + as_word off0)%w @Invalidated). eexists. split; auto.
               rewrite eq_refl. repeat rewrite setmE in Heqa40. repeat unfold_match' Heqa40; convert_eq_op.
               simplify_some. rewrite <- Heqa39 in RW0. simplify_some. reflexivity.
               rewrite <- Heqa40 in RW0. simplify_some. reflexivity. }
             { simpl. unfold as_word, ssrint.absz in *. simpl in *.
               repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end).
               simpl. subst pc3_val. simpl. eexists. split; auto. split; auto. }
             assert (r_in : In w' (domm reg0)).
             { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). exists d. trivial. }
             exfalso. rewrite reg_domm1 in r_in. simpl in r_in. unfold word_of_nat in r_in. simpl in r_in.
               move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
              repeat (destruct r_in as [|r_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
              inversion r_in. }
        (* -- inversion weak; trivial. *)
        -- unfold side_of. rewrite ip_comp'. trivial.
        -- inversion weak; trivial.
        -- inversion weak; trivial.
        -- inversion weak; trivial.
        -- unfold register_address_correctness. simpl.
           intros d r1 r_d_eq.
           unfold register_domm in *.
           assert (r_in : In r1 (domm regs3)).
           { clear - reg_domm3 r_d_eq vt_eq10. rewrite reg_domm3 -Extra.In_in in_fset; simpl.
             repeat (rewrite setmE in r_d_eq; unfold_match' r_d_eq; [ convert_eq_op |]; simpl in r_d_eq).
             all: try (red; simpl; tauto).
             rewrite -in_fset -reg_domm3. setoid_rewrite <- (rwP dommP). eauto.
             rewrite -in_fset -reg_domm3. setoid_rewrite <- (rwP dommP). eauto. }
           rewrite reg_domm3 in r_in.
           move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
           repeat (destruct r_in as [? | r_in]; try subst r1; repeat (rewrite setmE in r_d_eq; simpl in r_d_eq));
             simpl in r_d_eq; repeat simplify_some; simpl; trivial. unfold_match' r_d_eq.
           { destruct d; unfold is_other in *; unfold_match' Heqa51. }
           inversion r_in.
        -- unfold memory_match. simpl. inversion weak as [? ? ? _ _ _ _ _ _ mem_match']. subst.
           intros w' d d_code d_ip. split; intro w_eq.
           { destruct (fst (mem_match' w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H15; subst. destruct d''t; try exact H15. destruct H15 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H15. subst n vala1 vala0.
             match (type of d''eq) with _ = Some (?v) => assert (rel: is_relevant_comp Right (LRC.color (taga v))) by (simpl; auto) end.
             inversion weak as [? ? ? _ _ m_cor_s m_cor_s' _ _ _].
             destruct (m_cor_s _ _ rel d''eq) as [d' [d'eq d'code]]. simpl in d'eq.
             pose proof (capa_cor2 v15 (n1.-1) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. eauto.
             eapply (unicity (inl _)); eauto. rewrite H14. eauto. }
           { destruct (snd (mem_match' w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H15; subst. destruct d''t; try exact H15. destruct H15 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H15. subst n vala1 vala0.
             pose proof (capa_cor3 (w0 + as_word off0)%w (n1.-1) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. eauto.
             eapply (unicity (inl _)); eauto. }
    + (* return from comp in P to comp' in C *)
      repeat
        (match goal with
           H : Some ?a = getm reg0 _ |- _ => deduce_equality (esym H); pose proof (esym H); clear H
         end).
      deduce_equality RW0.
      deduce_equality RA0.
      subst s1. unfold side_of in *. unfold_match' side_eq. simpl in *.
      deduce_equality PC0.
      assert (Meq: exists top M', M = top :: M').
      { destruct vt_match10 as [ret_eq stack_prop]. unfold color_of in stack_prop.
        subst t10. simpl in stack_prop. destruct stack_prop as [top [col top_in]].
        destruct M; eauto. inversion top_in. }
      destruct Meq as [top [M' M_eq]].
      destruct vt_match10 as [ret_eq stack_prop].
      subst t10. simpl in stack_prop. destruct stack_prop as [s2val [comp'' triple_in]].
      match (type of triple_in) with (_ ?t _) => assert (top = t); [|subst top] end.
      { inv tag_pc1. simpl in *. subst.
        clear- wfst triple_in. destruct triple_in as [?| is_in]; auto. exfalso.
        remember (Nat.pred n) as n'. assert (n'_ineq: S n' >= n) by (subst; clear; induction n; eauto).
        clear Heqn'.
        revert is_in. revert n'_ineq. revert w0 s2val v11 comp'' n'.
        inversion wfst; subst; clear - WF_ST; simpl; unfold Init.Nat.pred, "+", addn_rec; rewrite Nat.add_1_r.
        all: induction WF_ST; intros a v b col n' n'_ineq is_in; inversion is_in.
        all: try (inversion H; subst n; (assert false; [|done]); rewrite <- (ltn_subrR 0 (S n')); unfold subn, subn_rec, Nat.sub;
                  unfold addn, addn_rec in n'_ineq; rewrite Nat.add_1_r in n'_ineq; done).
        all: eapply (IHWF_ST _ _ _ _ _ _ H). }
      assert (comp'' = comp'); [|subst comp''].
      { clear - wfst M_eq Heqa7 tag_mem_w0. inversion wfst; subst M; inv H0;
        unfold points_to_comp_code' in PTS_CODE1; oapp_False; simpl in *; rewrite <- Heqa7 in HeqH; simplify_some;
        destruct PTS_CODE1 as [col ?]; simpl in col; auto. }
      inversion wfst; subst M; try (inv H0; done); simpl in *.
      1: { exfalso. inv H14.
           eapply (@Machine.Intermediate.fdisjoint_partition_notinboth _ (domm ip) (domm ic)); eauto.
           inversion Hmergeable_ifaces as [[_ fdisj] _]; eauto. }
      inversion H14. subst v14 n n0 s2val v16 C st. clear H20 H14.
      unfold points_to_comp_code in PTS_CODE3. oapp_False.
      eexists. exists M'.
      split.
      * eapply (plus_left _ [:: ERet comp rcom_value1 comp'] ). try eapply step_jump; eauto.
        -- rewrite eq_s3 in vt_eq12. exact vt_eq12.
        -- inversion vt_match12; subst; revert H11; try decode_instr_eq.
        -- rewrite eq_s3 in vt_eq10. exact vt_eq10.
        -- rewrite eq_s3 in vt_eq11. exact vt_eq11.
        -- unfold reg_clear_list, reg_list. simpl. unfold as_word. subst. simpl.
           repeat
             (match goal with
              | H: _ ?w = _  |- context[_ ?w] => rewrite H; simpl
              end). trivial.
        -- unfold next_state_updates, next_state_updates_and_pc, next_state, transfer, instr_rules, LRC.instr_rules in *.
           unfold evi in *. simpl. unfold_bind.
           deduce_reg reg_match. subst v11. simpl in *.
           rewrite <- HeqH11. simpl. rewrite eq_s3 in tag_pc3. simpl in tag_pc3.
           rewrite tag_pc3. rewrite eq_refl. simpl.
           destruct PTS_CODE3 as [col_a10 code_a10]. destruct a10 as [va10 ta10]. destruct ta10.
           simpl in col_a10. rewrite col_a10. simpl. rewrite <- Heqa10. simpl.
           destruct a23 as [? ta]. unfold is_other in *. unfold_match' Heqa51. simpl in *.
           subst ta. inversion vt_match9 as [? _]. subst t9. simpl.
           unfold "+", addn_rec. rewrite Nat.add_1_r. simpl. rewrite eq_refl. simpl.
           unfold updm. unfold as_word. simpl in *.
           (*automatically rewrite regs3 registers*)
           repeat
             (repeat
                (match goal with
                 | H: getm ?r ?w = _  |- context[getm ?r ?w] => rewrite H; simpl
                 end);
              (*automatically transforms setm in if (_ == _) then _ else _*)
              repeat rewrite setmE; simpl;
              (*destruct the (_ == _) condition *)
              try
                (let cond := fresh "cond" in
                 match goal with
                 | |- context[@eq_op ?t ?a ?r] =>
                     remember (@eq_op t a r) as cond; destruct cond;
                     [convert_eq_op; simpl in *|]; simpl
                 end)).
           all: inversion Heqa0; subst rcom_value1; rewrite H10 in Heqa5; simplify_some; destruct x; destruct H11; subst; simpl.
           13: reflexivity. (* most general case, to be treated first *)
           all: unfold as_word in *; simpl in *.
           all: (match (type of vt_eq10) with
                 | ?ls = ?rs =>
                     revert vt_eq10;
                     (match goal with
                        Heq: ls = Some (?a) |- _ =>
                          intro vt_eq10; rewrite Heq in vt_eq10; simplify_some; subst; simpl
                      end)
                 end).
           all: repeat (rewrite setmxx || (rewrite (setmC _ ((addw _ _)@_)); [|done]) ).
           all: trivial.
        -- eapply star_refl.
        -- reflexivity.
      * inversion tag_pc1. rewrite eq_s3 in tag_pc3. simpl in tag_pc3. rewrite <- H14 in tag_pc3. subst pc3_tag.
        clear tag_pc1. subst. simpl in *.
        repeat rewrite <- H14.
        eapply match_states_right; econstructor; simpl; unfold build_tpc; try trivial.
        -- unfold register_domm. repeat rewrite domm_set. simpl.
           unfold register_domm, reg_field_size, mword, FSet.fsval in reg_domm1. simpl in reg_domm1.
           unfold mword, word_size. simpl.
           (* clear - reg_domm1 RW0. admit. (* exstructure *) *)
           clear -reg_domm1 RW0.
           rewrite //= reg_domm1 //=.
           repeat match goal with
             | |- context [as_word (ssrint.Posz ?n)] =>
                 replace (@as_word 5 (ssrint.Posz n)) with
                 (@word_of_nat 5 n) by reflexivity
           end.
           rewrite <- eq_fset.
           intros r. rewrite in_fset.
           match goal with
           | |- (r \in (word_of_nat _ |: _)%fset) = (r \in _) =>
               rewrite in_fset in_cons
           end;
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=| clear eqr]
           end.
           repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
           rewrite in_fset in_cons.
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
           end.
           ++ by rewrite -in_fset -reg_domm1 mem_domm RW0.
           ++ repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
              now rewrite in_fset.
        -- unfold register_domm. repeat rewrite domm_set. simpl.
           unfold register_domm, reg_field_size, mword, FSet.fsval in reg_domm2. simpl in reg_domm2.
           unfold mword, word_size. simpl.
           clear -reg_domm2 RW.
           rewrite //= reg_domm2 //=.
           repeat match goal with
             | |- context [as_word (ssrint.Posz ?n)] =>
                 replace (@as_word 5 (ssrint.Posz n)) with
                 (@word_of_nat 5 n) by reflexivity
           end.
           rewrite <- eq_fset. rename r into r0.
           intros r. rewrite in_fset.
           match goal with
           | |- (r \in (word_of_nat _ |: _)%fset) = (r \in _) =>
               rewrite in_fset in_cons
           end;
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=| clear eqr]
           end.
           repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
           rewrite in_fset in_cons.
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
           end.
           ++ by rewrite -in_fset -reg_domm2 mem_domm RW.
           ++ repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
              now rewrite in_fset.
        -- unfold register_domm. repeat rewrite domm_set. simpl.
           unfold register_domm, reg_field_size, mword, FSet.fsval in reg_domm3. simpl in reg_domm3.
           unfold mword, word_size. simpl.
           clear - reg_domm3 vt_eq10.
           rewrite //= reg_domm3 //=.
           repeat match goal with
             | |- context [as_word (ssrint.Posz ?n)] =>
                 replace (@as_word 5 (ssrint.Posz n)) with
                 (@word_of_nat 5 n) by reflexivity
           end.
           unfold word_of_nat. unfold as_word. simpl.
           rewrite <- eq_fset.
           intros r. rewrite in_fset.
           match goal with
           | |- (r \in (Word _ |: _)%fset) = (r \in _) =>
               rewrite in_fset in_cons
           end;
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=| clear eqr]
           end.
           repeat (match goal with
                   | |- (r \in FSet.fsval (Word _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
           rewrite in_fset in_cons.
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
           end.
           ++ by rewrite -in_fset -reg_domm3 mem_domm vt_eq10.
           ++ repeat (match goal with
                   | |- (r \in FSet.fsval (Word _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
              now rewrite in_fset.
        -- split; [|split; [|split; [|split; [|split; [|split; [| split]]]]]]; trivial.
        -- split; [|split; [|split; [|split; [|split; [|split; [| split]]]]]]; trivial.
        -- split; [|split; [|split; [|split; [|split; [|split; [| split]]]]]]; trivial.
        -- intros v' n rw rweq. simpl in *. destruct rw as [r' | w'].
           { exfalso. repeat rewrite setmE in rweq. repeat unfold_match' rweq.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               now rewrite -in_fset -reg_domm1 mem_domm rweq.
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in. }
           pose proof (capa_cor1 v' n (inr w') rweq) as [is_in unicity]. simpl in unicity. split.
           { unfold in_stack in *. destruct is_in as [sv1 [sv2 [col [|is_in]]]]; [|eauto].
             exfalso. inversion H11. subst n v' sv1 sv2 col. eapply (unicity (inl _)). exact RW0. }
           intros rw v''. destruct rw as [r' | w'']; simpl.
           { intro rweq'. exfalso. repeat rewrite setmE in rweq'. repeat unfold_match' rweq'.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               now rewrite -in_fset -reg_domm1 mem_domm rweq'.
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in.  }
           intros [t' [v''eq t'eq]]. eapply (unicity (inr _)); eauto.
        -- intros v' n rw rweq. simpl in *. destruct rw as [r' | w'].
           { exfalso. repeat rewrite setmE in rweq. repeat unfold_match' rweq.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               now rewrite -in_fset -reg_domm2 mem_domm rweq.
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in. }
           pose proof (capa_cor2 v' n (inr w') rweq) as [is_in unicity]. simpl in unicity. split.
           { unfold in_stack in *. destruct is_in as [sv1 [sv2 [col [|is_in]]]]; [|eauto].
             exfalso. inversion H11. subst n v' sv1 sv2 col. eapply (unicity (inl _)). rewrite H14. exact RW. }
           intros rw v''. destruct rw as [r' | w'']; simpl.
           { intro rweq'. exfalso. repeat rewrite setmE in rweq'. repeat unfold_match' rweq'.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               now rewrite -in_fset -reg_domm2 mem_domm rweq'.
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in.  }
           intros [t' [v''eq t'eq]]. eapply (unicity (inr _)); eauto.
        -- intros v' n rw rweq. simpl in *. destruct rw as [r' | w'].
           { exfalso. repeat rewrite setmE in rweq. repeat unfold_match' rweq.
             repeat simplify_some. inversion Heqa51.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               now rewrite -in_fset -reg_domm3 mem_domm rweq.
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold as_word in r'_in. simpl in r'_in.
             unfold word_of_nat, as_word in r'_in. simpl in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in. }
           pose proof (capa_cor3 v' n (inr w') rweq) as [is_in unicity]. simpl in unicity. split.
           { unfold in_stack in *. destruct is_in as [sv1 [sv2 [col [|is_in]]]]; [|eauto].
             exfalso. inversion H11. subst n v' sv1 sv2 col. eapply (unicity (inl _)). exact vt_eq10. }
           intros rw v''. destruct rw as [r' | w'']; simpl.
           { intro rweq'. exfalso. repeat rewrite setmE in rweq'. repeat unfold_match' rweq'.
             repeat simplify_some. inversion Heqa51.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               now rewrite -in_fset -reg_domm3 mem_domm rweq'.
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. simpl in r'_in. unfold word_of_nat, as_word in r'_in. simpl in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in.  }
           intros [t' [v''eq t'eq]]. eapply (unicity (inr _)); eauto.
        -- unfold side_of. goal_match_bind_step; trivial.
           exfalso. eapply (@Machine.Intermediate.fdisjoint_partition_notinboth _ (domm ip) (domm ic)); eauto.
           inversion Hmergeable_ifaces as [[_ fdisj] _]; eauto.
        -- unfold register_address_correctness. simpl.
           intros d r1 r_d_eq. repeat rewrite setmE in r_d_eq. repeat unfold_match' r_d_eq.
           { convert_eq_op. simplify_some. destruct (taga d); inversion Heqa51. simpl. trivial. }
           unfold register_domm in *.
           assert (r_in : In r1 (domm regs3)).
           { clear - reg_domm3 r_d_eq vt_eq12. rewrite reg_domm3.
             repeat (rewrite setmE in r_d_eq; unfold_match' r_d_eq; [ convert_eq_op |]; simpl in r_d_eq).
             all: try (simpl; tauto).
             rewrite <- reg_domm3. setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto. }
           rewrite reg_domm3 in r_in. simpl in r_in. unfold word_of_nat, as_word in r_in. simpl in r_in.
           move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
           repeat (destruct r_in as [|r_in];
                   [subst;
                    (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
           inversion r_in.
        -- unfold memory_match. simpl. inversion weak as [? ? ? _ _ m_cor_s3 m_cor_s2 _ _ _]. subst.
           intros w' d d_code d_ip.
           split; intro w_eq.
           { destruct (fst (mem_match w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H15; subst. destruct d''t; try exact H15. destruct H15 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H15. subst n vala1 vala0.
             match (type of d''eq) with _ = Some (?v) => assert (rel: is_relevant_comp Left (LRC.color (taga v))) by (simpl; auto) end.
             destruct (s_mem_cor _ _ rel d''eq) as [d' [d'eq d'code]]. simpl in d'eq.
             pose proof (capa_cor1 w0 (n1.-1) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. eauto.
             eapply (unicity (inl _)); eauto. }
           { destruct (snd (mem_match w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H15; subst. destruct d''t; try exact H15. destruct H15 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H15. subst n vala1 vala0.
             pose proof (capa_cor3 (v15 + as_word off0)%w (n1.-1) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. eauto.
             eapply (unicity (inl _)); eauto. }
        (* -- inversion weak; trivial. *)
        -- unfold_match' Heqa2. rewrite <- H14 in RW.
           pose proof (capa_cor2 w (n1.-1) (inl r) RW) as [is_in unicity].
           unfold in_stack in is_in. simpl in is_in.
           eapply same_pc_normal; simpl; eauto.
           destruct is_in as [? [? [? [eq | is_in]]]]. inv eq; trivial.
           exfalso. clear -is_in WF_ST. remember (n1.-1) as n. clear Heqn n1.
           revert is_in WF_ST. revert M'.
           induction n; intros M' is_in wfst.
           ++ inversion wfst; subst; try (inversion is_in; done); rewrite addn1 in H0; inversion H0.
           ++ simpl in *. inversion wfst; subst; try (inversion is_in; done); rewrite addn1 in H0; inv H0;
              eapply (IHn _ _ WF_ST). (* TODO: why does this work at all? *)
        -- unfold side_of. goal_match_bind_step; auto.
           exfalso. eapply (@Machine.Intermediate.fdisjoint_partition_notinboth _ (domm ip) (domm ic)); eauto.
           inversion Hmergeable_ifaces as [[_ fdisj] _]; eauto.
        -- inversion weak; trivial.
        -- inversion weak; trivial.
        -- inversion weak; trivial.
        -- unfold register_address_correctness. simpl.
           intros d r1 r_d_eq. repeat rewrite setmE in r_d_eq. repeat unfold_match' r_d_eq.
           unfold register_domm in *.
           assert (r_in : In r1 (domm reg)).
           { clear - reg_domm2 r_d_eq RW0. rewrite reg_domm2.
             repeat (rewrite setmE in r_d_eq; unfold_match' r_d_eq; [ convert_eq_op |]; simpl in r_d_eq).
             all: try (simpl; tauto).
             rewrite <- reg_domm2. setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto. }
           rewrite reg_domm2 in r_in. unfold word_of_nat in r_in. simpl in r_in.
           move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
           repeat (destruct r_in as [|r_in];
                   [subst;
                    (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
           inversion r_in.
        -- unfold register_address_correctness. simpl.
           intros d r1 r_d_eq.
           unfold register_domm in *.
           assert (r_in : In r1 (domm regs3)).
           { clear - reg_domm3 r_d_eq vt_eq10. rewrite reg_domm3 -Extra.In_in in_fset; simpl.
             repeat (rewrite setmE in r_d_eq; unfold_match' r_d_eq; [ convert_eq_op |]; simpl in r_d_eq).
             all: try (red; simpl; tauto).
             rewrite -in_fset -reg_domm3. setoid_rewrite <- (rwP dommP). eauto.
             rewrite -in_fset -reg_domm3. setoid_rewrite <- (rwP dommP). eauto. }
           rewrite reg_domm3 in r_in.
           move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
           repeat (destruct r_in as [? | r_in]; try subst r1; repeat (rewrite setmE in r_d_eq; simpl in r_d_eq));
             simpl in r_d_eq; repeat simplify_some; simpl; trivial. unfold_match' r_d_eq.
           { destruct d; unfold is_other in *; unfold_match' Heqa51. }
           inversion r_in.
        -- unfold memory_match. simpl. inversion weak as [? ? ? _ _ m_cor_s2 m_cor_s3 _ _ mem_match']. subst.
           intros w' d d_code d_ip. split; intro w_eq.
           { destruct (fst (mem_match' w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H15; subst. destruct d''t; try exact H15. destruct H15 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H15. subst n vala1 vala0.
             match (type of d''eq) with _ = Some (?v) => assert (rel: is_relevant_comp Right (LRC.color (taga v))) by (simpl; auto) end.
             destruct (m_cor_s2 _ _ rel d''eq) as [d' [d'eq d'code]]. simpl in d'eq.
             pose proof (capa_cor2 v15 (n1.-1) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. eauto.
             eapply (unicity (inl _)); eauto. rewrite H14. exact RW. }
           { destruct (snd (mem_match' w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H15; subst. destruct d''t; try exact H15. destruct H15 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H15. subst n vala1 vala0.
             match (type of d''eq) with _ = Some (?v) => assert (rel: is_relevant_comp Right (LRC.color (taga v))) by (simpl; auto) end.
             subst. destruct (m_cor_s3 _ _ rel d''eq) as [d' [d'eq d'code]]. simpl in d'eq.
             pose proof (capa_cor3 (v15 + as_word off0)%w (n1.-1) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. eauto.
             eapply (unicity (inl _)); eauto. }
        -- unfold registers_match. simpl. intros w' d.
           split; intro w_eq.
           { repeat (rewrite setmE in w_eq; unfold_match' w_eq; [ convert_eq_op |]; simpl in w_eq).
             all: try simplify_some.
             all: repeat (rewrite setmE; simpl).
             1-11: eexists; split; [| reflexivity].
             1-11: try (split; trivial).
             { rewrite Heqa in Heqa0. unfold evi in Heqa0. do 2 unfold_bind; [|rewrite H10 in Heqa30; inversion Heqa30].
               simplify_some. simpl in Heqa30.
               rewrite H10 in Heqa30. simplify_some.
               destruct d as [v_d t_d]. unfold is_other in *. destruct t_d; inversion Heqa51.
               split; trivial.
               match goal with |- (_ (match ?c with _ => _ end) = _) => destruct c eqn:req end.
               { pose proof (esym req). convert_eq_op. simpl. simpl in a21_eq.
                 unfold as_word in Heqa31. simpl in Heqa31. rewrite <- a21_eq in Heqa31. simplify_some. simpl in H15.
                 destruct a21 as [v_a ?]. clear - H15. simpl in *.
                 eapply convert_int_of_word_inj; eauto. }
               { unfold as_word in Heqa31. simpl in Heqa31. rewrite <- Heqa31 in Heqa9. simplify_some. simpl in H15.
                 destruct a12 as [v_a ?]. clear - H15. simpl in *.
                 eapply convert_int_of_word_inj; eauto. }
             }
             { simpl. unfold as_word. unfold ssrint.absz. simpl.
               repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end).
               simpl. subst pc3_val. simpl.
               eexists (if (@eq_op (Ord.eqType _) r0 r) then _ else _)@Invalidated.
               split.
               2: { simpl. goal_match_bind_step. trivial.
                 goal_match_bind_step. trivial.
                 exfalso.  assert (r_in : In r0 (domm reg0)).
                 { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto. }
                 rewrite reg_domm1 in r_in. simpl in r_in. unfold word_of_nat, as_word in r_in. simpl in r_in.
                 move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
                 repeat (destruct r_in as [|r_in];
                         [subst;
                          (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
                 inversion r_in. }
               split; auto. }
             { exists (Types.vala a22)@Invalidated. split; [split; auto|].
               goal_match_bind_step; auto. convert_eq_op. simpl in a21_eq. simplify_some. trivial. }
             { assert (r_in : In w' (domm regs3)).
               { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). exists d. trivial. }
               exfalso. rewrite reg_domm3 in r_in. simpl in r_in. unfold word_of_nat, as_word in r_in. simpl in r_in.
               move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
               repeat (destruct r_in as [|r_in];
                       [subst;
                        (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
               inversion r_in. } }
           { repeat (rewrite setmE in w_eq; unfold_match' w_eq; [ convert_eq_op |]; simpl in w_eq).
             all: try simplify_some.
             all: repeat (rewrite setmE; simpl).
             1-10: eexists; split; [| reflexivity].
             1-10: try (split; trivial).
             { rewrite Heqa in Heqa0. unfold evi in Heqa0. do 2 unfold_bind; [|rewrite H10 in Heqa30; inversion Heqa30].
               simplify_some. simpl in Heqa30.
               rewrite H10 in Heqa30. simplify_some.
               destruct a23 as [v_d t_d]. unfold is_other in *. destruct t_d; inversion Heqa51.
               eexists. split; trivial. split; trivial.
               unfold_match' a21_eq; simplify_some; trivial; simpl; convert_eq_op; simpl.
               { unfold as_word in Heqa31. simpl in Heqa31. rewrite <- Heqa31 in Heqa9. simplify_some. simpl in H15.
                 destruct a12 as [v_a ?]. clear - H15. simpl in *.
                 eapply convert_int_of_word_inj; eauto. }
               match goal with |- (_ (match ?c with _ => _ end) = _) => destruct c eqn:req end.
               { pose proof (esym req). convert_eq_op. simpl. simpl in a21_eq.
                 unfold as_word in Heqa31. simpl in Heqa31. rewrite <- a21_eq in Heqa31. simplify_some. simpl in H15.
                 destruct a21 as [v_a ?]. clear - H15. simpl in *.
                 eapply convert_int_of_word_inj; eauto. }
               { unfold as_word in Heqa31. simpl in Heqa31. rewrite <- Heqa31 in Heqa9. simplify_some. simpl in H15.
                 destruct a12 as [v_a ?]. clear - H15. simpl in *.
                 eapply convert_int_of_word_inj; eauto. }
             }
             { simpl. unfold as_word, ssrint.absz in *. simpl in *.
               repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end).
               simpl. subst pc3_val. simpl.
               (*eexists ((Types.vala a45 + as_word off0)%w @Invalidated). eexists. split; auto. *)
               eexists (if (@eq_op (Ord.eqType _) r r0) then _ else _)@Invalidated.
               split.
               2: { simpl. goal_match_bind_step. trivial.
                    goal_match_bind_step. trivial.
                    exfalso.  assert (r_in : In r (domm reg)).
                    { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto. }
                    rewrite reg_domm2 in r_in. simpl in r_in. unfold word_of_nat, as_word in r_in. simpl in r_in.
                    move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
                    repeat (destruct r_in as [|r_in];
                            [subst;
                             (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
                    inversion r_in. }
               split; trivial. }
               { simpl. unfold as_word, ssrint.absz in *. simpl in *.
                 repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end).
                 simpl. subst pc3_val. simpl.
                 eexists (if (@eq_op (Ord.eqType _) (as_word (ssrint.Posz 4)) r0) then _ else _)@Invalidated. split.
                 2: { unfold as_word. simpl. goal_match_bind_step. trivial. trivial. }
                 split; trivial. }
               {
                 assert (r_in : In w' (domm reg)).
                 { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). exists d. trivial. }
                 exfalso. rewrite reg_domm2 in r_in. simpl in r_in. unfold word_of_nat in r_in. simpl in r_in.
                 move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
                 repeat (destruct r_in as [|r_in];
                         [subst;
                          (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
                 inversion r_in. }
           }
    + (* return from comp in C to comp' in P *)
      repeat
      (match goal with
         H : Some ?a = getm reg _ |- _ => deduce_equality (esym H); pose proof (esym H); clear H
       end).
      deduce_equality RW.
      deduce_equality RA.
      subst s2. unfold side_of in *. unfold_match' side_eq. simpl in *.
      assert (side_eq': true = (comp \in domm ic)).
      { assert (comp_incl: comp \in (domm ic) \/ comp \in (domm ip)) by
          (rewrite ST0 in col_mem1; pose proof (col_mem1 _ _ PC0) as in_disj; simpl in in_disj;
           setoid_rewrite <- Extra.In_in in in_disj; exact ((fst (or_comm _ _)) in_disj)).
        destruct comp_incl as [Hl | Hr]. rewrite Hl. trivial. rewrite Hr in Heqa5. inversion Heqa5. }
      deduce_equality PC.
      assert (Meq: exists top M', M = top :: M').
      { destruct vt_match10 as [ret_eq stack_prop]. unfold color_of in stack_prop.
        subst t10. simpl in stack_prop. destruct stack_prop as [top [col top_in]].
        destruct M; eauto. inversion top_in. }
      destruct Meq as [top [M' M_eq]].
      destruct vt_match10 as [ret_eq stack_prop].
      subst t10. simpl in stack_prop. destruct stack_prop as [s2val [comp'' triple_in]].
      match (type of triple_in) with (_ ?t _) => assert (top = t); [|subst top] end.
      { inv tag_pc1. simpl in *. subst.
        clear- wfst triple_in. destruct triple_in as [?| is_in]; auto. exfalso.
        remember (Nat.pred n) as n'. assert (n'_ineq: S n' >= n) by (subst; clear; induction n; eauto).
        clear Heqn'. revert is_in. revert n'_ineq. revert w s2val v11 comp'' n'.
        inversion wfst; subst; clear - WF_ST; simpl; unfold Init.Nat.pred, "+", addn_rec; rewrite Nat.add_1_r.
        all: induction WF_ST; intros a v b col n' n'_ineq is_in; inversion is_in.
        all: try (inversion H; subst n; (assert false; [|done]); rewrite <- (ltn_subrR 0 (S n')); unfold subn, subn_rec, Nat.sub;
                  unfold addn, addn_rec in n'_ineq; rewrite Nat.add_1_r in n'_ineq; done).
        all: eapply (IHWF_ST _ _ _ _ _ _ H). }
      assert (comp'' = comp'); [|subst comp''].
      { unfold_match' Heqa2. inversion Heqa2 as [tag_mem_w].
        clear - wfst M_eq Heqa6 tag_mem_w. destruct a10. simpl in *. subst taga.
        inversion wfst; subst M; inv H0; try rename PTS_COD22 into PTS_CODE2;
        unfold points_to_comp_code in PTS_CODE2; oapp_False; simpl in *; rewrite <- Heqa6 in HeqH; simplify_some;
        destruct PTS_CODE2 as [col ?]; simpl in col; auto. }
      inversion wfst; subst M; try (inv H0; done); simpl in *.
      2: { exfalso. inv H13.
           eapply (@Machine.Intermediate.fdisjoint_partition_notinboth _ (domm ip) (domm ic)); eauto.
           inversion Hmergeable_ifaces as [[_ fdisj] _]; eauto. }
      inversion H13. subst n s2val v15 v16 C st. clear H19 H13.
      unfold points_to_comp_code in PTS_CODE3. oapp_False.
      eexists. exists M'.
      split.
      * eapply (plus_left _ [:: ERet comp rcom_value1 comp'] ). try eapply step_jump; eauto.
        -- rewrite eq_s3 in vt_eq12. exact vt_eq12.
        -- inversion vt_match12; subst; revert H12; try decode_instr_eq.
        -- rewrite eq_s3 in vt_eq10. exact vt_eq10.
        -- rewrite eq_s3 in vt_eq11. exact vt_eq11.
        -- unfold reg_clear_list, reg_list. simpl. unfold as_word. subst. simpl.
           repeat
             (match goal with
              | H: _ ?w = _  |- context[_ ?w] => rewrite H; simpl
              end). trivial.
        -- unfold next_state_updates, next_state_updates_and_pc, next_state, transfer, instr_rules, LRC.instr_rules in *.
           unfold evi in *. simpl. unfold_bind.
           deduce_reg reg_match. simpl in *. rewrite <- H18.
           rewrite <- HeqH12. simpl. rewrite eq_s3 in tag_pc3. simpl in tag_pc3.
           rewrite tag_pc3. rewrite eq_refl. simpl.
           destruct PTS_CODE3 as [col_a10 code_a10]. destruct a10 as [va10 ta10]. destruct ta10.
           simpl in col_a10. rewrite col_a10. simpl. rewrite eq_refl. simpl. rewrite <- Heqa35. simpl.
           destruct a as [? ta]. unfold is_other in *. unfold_match' Heqa36. simpl in *.
           subst ta. inversion vt_match9 as [? _]. subst t9. simpl.
           unfold updm. unfold as_word. simpl in *.
           (*automatically rewrite regs3 registers*)
           repeat
             (repeat
                (match goal with
                 | H: getm ?r ?w = _  |- context[getm ?r ?w] => rewrite H; simpl
                 end);
              (*automatically transforms setm in if (_ == _) then _ else _*)
              repeat rewrite setmE; simpl;
              (*destruct the (_ == _) condition *)
              try
                (let cond := fresh "cond" in
                 match goal with
                 | |- context[@eq_op ?t ?a ?r] =>
                     remember (@eq_op t a r) as cond; destruct cond;
                     [convert_eq_op; simpl in *|]; simpl
                 end)).
           all: inversion Heqa0; subst rcom_value1; rewrite ST0 in Heqa11; simpl in Heqa11; rewrite H11 in Heqa;
             simpl in Heqa; inversion Heqa; rewrite H14; destruct x, H12; subst; simpl.
           13: reflexivity. (* most general case, to be treated first *)
           all: (rewrite (setmxx _ _ (v12@Invalidated) _ )) || (rewrite (setmC _ (v12@Invalidated)); [|done]).
           all: unfold as_word in *; simpl in *.
           all: (match (type of vt_eq10) with
                 | ?ls = ?rs =>
                     revert vt_eq10;
                     (match goal with
                        Heq: ls = Some (?a) |- _ =>
                          intro vt_eq10; rewrite Heq in vt_eq10; simplify_some; subst; simpl
                      end)
                 end).
           all: repeat (rewrite setmxx || (rewrite (setmC _ ((addw _ _)@_)); [|done]) ).
           all: trivial.
           repeat ((rewrite (setmC _ (v12@_))); [|done]). simplify_some. trivial.
        -- eapply star_refl.
        -- reflexivity.
      * rewrite ST0 in tag_pc1. simpl in tag_pc1. inversion tag_pc1. subst n1. clear tag_pc1.
        rewrite eq_s3 in tag_pc3. simpl in tag_pc3. subst pc3_tag. subst. simpl in *.
        repeat rewrite <- H17.
        eapply match_states_left; econstructor; simpl; unfold build_tpc; try trivial.
        -- unfold register_domm. repeat rewrite domm_set. simpl.
           unfold register_domm, reg_field_size, mword, FSet.fsval in reg_domm1. simpl in reg_domm1.
           unfold mword, word_size. simpl.
           (* clear - reg_domm1 RW0. admit. (* exstructure *) *)
           clear -reg_domm1 RW0.
           rewrite //= reg_domm1 //=.
           repeat match goal with
             | |- context [as_word (ssrint.Posz ?n)] =>
                 replace (@as_word 5 (ssrint.Posz n)) with
                 (@word_of_nat 5 n) by reflexivity
           end.
           rewrite <- eq_fset.
           intros r. rewrite in_fset.
           match goal with
           | |- (r \in (word_of_nat _ |: _)%fset) = (r \in _) =>
               rewrite in_fset in_cons
           end;
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=| clear eqr]
           end.
           repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
           rewrite in_fset in_cons.
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
           end.
           ++ by rewrite -in_fset -reg_domm1 mem_domm RW0.
           ++ repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
              now rewrite in_fset.
        -- unfold register_domm. repeat rewrite domm_set. simpl.
           unfold register_domm, reg_field_size, mword, FSet.fsval in reg_domm2. simpl in reg_domm2.
           unfold mword, word_size. simpl.
           clear -reg_domm2 RW.
           rewrite //= reg_domm2 //=.
           repeat match goal with
             | |- context [as_word (ssrint.Posz ?n)] =>
                 replace (@as_word 5 (ssrint.Posz n)) with
                 (@word_of_nat 5 n) by reflexivity
           end.
           rewrite <- eq_fset. rename r into r0.
           intros r. rewrite in_fset.
           match goal with
           | |- (r \in (word_of_nat _ |: _)%fset) = (r \in _) =>
               rewrite in_fset in_cons
           end;
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=| clear eqr]
           end.
           repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
           rewrite in_fset in_cons.
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
           end.
           ++ by rewrite -in_fset -reg_domm2 mem_domm RW.
           ++ repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
              now rewrite in_fset.
        -- unfold register_domm. repeat rewrite domm_set. simpl.
           unfold register_domm, reg_field_size, mword, FSet.fsval in reg_domm3. simpl in reg_domm3.
           unfold mword, word_size. simpl.
           clear - reg_domm3 vt_eq10.
           rewrite //= reg_domm3 //=.
           repeat match goal with
             | |- context [as_word (ssrint.Posz ?n)] =>
                 replace (@as_word 5 (ssrint.Posz n)) with
                 (@word_of_nat 5 n) by reflexivity
           end.
           unfold word_of_nat. unfold as_word. simpl.
           rewrite <- eq_fset. rename r into r0.
           intros r. rewrite in_fset.
           match goal with
           | |- (r \in (Word _ |: _)%fset) = (r \in _) =>
               rewrite in_fset in_cons
           end;
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=| clear eqr]
           end.
           repeat (match goal with
                   | |- (r \in FSet.fsval (Word _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
           rewrite in_fset in_cons.
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
           end.
           ++ by rewrite -in_fset -reg_domm3 mem_domm vt_eq10.
           ++ repeat (match goal with
                   | |- (r \in FSet.fsval (Word _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
              now rewrite in_fset.
        -- split; [|split; [|split; [|split; [|split; [|split; [| split]]]]]]; trivial.
        -- split; [|split; [|split; [|split; [|split; [|split; [| split]]]]]]; trivial.
        -- split; [|split; [|split; [|split; [|split; [|split; [| split]]]]]]; trivial.
        -- intros v' n rw rweq. simpl in *. destruct rw as [r' | w'].
           { exfalso. repeat rewrite setmE in rweq. repeat unfold_match' rweq.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               (rewrite -in_fset -reg_domm1; rewrite mem_domm; rewrite rweq; trivial).
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in. }
           pose proof (capa_cor1 v' n (inr w') rweq) as [is_in unicity]. simpl in unicity. split.
           { unfold in_stack in *. destruct is_in as [sv1 [sv2 [col [|is_in]]]]; [|eauto].
             exfalso. inversion H12. subst n v' sv1 sv2 col. eapply (unicity (inl _)). rewrite H17. exact RW0. }
           intros rw v''. destruct rw as [r' | w'']; simpl.
           { intro rweq'. exfalso. repeat rewrite setmE in rweq'. repeat unfold_match' rweq'.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               (rewrite -in_fset -reg_domm1; rewrite mem_domm; rewrite rweq'; trivial).
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in.  }
           intros [t' [v''eq t'eq]]. eapply (unicity (inr _)); eauto.
        -- intros v' n rw rweq. simpl in *. destruct rw as [r' | w'].
           { exfalso. repeat rewrite setmE in rweq. repeat unfold_match' rweq.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               (rewrite -in_fset -reg_domm2; rewrite mem_domm; rewrite rweq; trivial).
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in. }
           pose proof (capa_cor2 v' n (inr w') rweq) as [is_in unicity]. simpl in unicity. split.
           { unfold in_stack in *. destruct is_in as [sv1 [sv2 [col [|is_in]]]]; [|eauto].
             exfalso. inversion H12. subst n v' sv1 sv2 col. eapply (unicity (inl _)). exact RW. }
           intros rw v''. destruct rw as [r' | w'']; simpl.
           { intro rweq'. exfalso. repeat rewrite setmE in rweq'. repeat unfold_match' rweq'.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               (rewrite -in_fset -reg_domm2; rewrite mem_domm; rewrite rweq'; trivial).
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in.  }
           intros [t' [v''eq t'eq]]. eapply (unicity (inr _)); eauto.
        -- intros v' n rw rweq. simpl in *. destruct rw as [r' | w'].
           { exfalso. repeat rewrite setmE in rweq. repeat unfold_match' rweq.
             repeat simplify_some. inversion Heqa36.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               (rewrite -in_fset -reg_domm3; rewrite mem_domm ; rewrite rweq; trivial).
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold as_word in r'_in. simpl in r'_in.
             unfold word_of_nat, as_word in r'_in. simpl in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in. }
           pose proof (capa_cor3 v' n (inr w') rweq) as [is_in unicity]. simpl in unicity. split.
           { unfold in_stack in *. destruct is_in as [sv1 [sv2 [col [|is_in]]]]; [|eauto].
             exfalso. inversion H12. subst n v' sv1 sv2 col. eapply (unicity (inl _)). exact vt_eq10. }
           intros rw v''. destruct rw as [r' | w'']; simpl.
           { intro rweq'. exfalso. repeat rewrite setmE in rweq'. repeat unfold_match' rweq'.
             repeat simplify_some. inversion Heqa36.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list)) by
               (rewrite -in_fset -reg_domm3; rewrite mem_domm; rewrite rweq'; trivial).
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. simpl in r'_in. unfold word_of_nat, as_word in r'_in. simpl in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in.  }
           intros [t' [v''eq t'eq]]. eapply (unicity (inr _)); eauto.
        (* -- inversion weak; trivial. *)
        -- unfold_match' Heqa2. rewrite <- H17 in RW.
           pose proof (capa_cor1 _ _ (inl r0) (RW0)) as [is_in unicity]. simpl in unicity.
           unfold in_stack in is_in. simpl in is_in.
           eapply same_pc_normal; simpl; eauto.
            destruct is_in as [? [? [? [eq | is_in]]]]. inv eq. trivial.
           exfalso. clear -is_in WF_ST.
           revert is_in WF_ST. revert M'.
           induction n0; intros M' is_in wfst.
           ++ inversion wfst; subst; try (inversion is_in; done); rewrite addn1 in H0; inversion H0.
           ++ simpl in *. inversion wfst; subst; try (inversion is_in; done); rewrite addn1 in H0; inv H0;
              eapply (IHn0 _ _ WF_ST). (* TODO: why does this work at all? *)
        -- unfold side_of. rewrite SIDE. trivial.
        -- inversion weak; trivial.
        -- inversion weak; trivial.
        -- inversion weak; trivial.
        -- unfold register_address_correctness. simpl.
           intros d r1 r_d_eq. repeat rewrite setmE in r_d_eq. repeat unfold_match' r_d_eq.
           unfold register_domm in *.
           assert (r_in : In r1 (domm reg0)).
           { clear - reg_domm2 r_d_eq. setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto. }
           rewrite reg_domm1 in r_in. unfold word_of_nat in r_in. simpl in r_in.
           move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
           repeat (destruct r_in as [|r_in];
                   [subst;
                    (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
           inversion r_in.
        -- unfold register_address_correctness. simpl.
           intros d r1 r_d_eq.
           unfold register_domm in *.
           assert (r_in : In r1 (domm regs3)).
           { clear - reg_domm3 r_d_eq vt_eq10. rewrite reg_domm3.
             repeat (rewrite setmE in r_d_eq; unfold_match' r_d_eq; [ convert_eq_op |]; simpl in r_d_eq).
             all: try (rewrite -Extra.In_in in_fset; red; simpl; tauto).
             rewrite <- reg_domm3. setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto.
             rewrite <- reg_domm3. setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). exists d. trivial. }
           rewrite reg_domm3 in r_in.
           move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
           repeat (destruct r_in as [? | r_in]; try subst r1; repeat (rewrite setmE in r_d_eq; simpl in r_d_eq));
             simpl in r_d_eq; repeat simplify_some; simpl; trivial. unfold_match' r_d_eq.
           { destruct d; unfold is_other in *; unfold_match' Heqa36. }
           inversion r_in.
        -- unfold memory_match. simpl. inversion weak as [? ? ? _ _ m_cor_s3 m_cor_s2 _ _ mem_match']. subst.
           intros w' d d_code d_ip.
           split; intro w_eq.
           { destruct (fst (mem_match' w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H13; subst. destruct d''t; try exact H13. destruct H13 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H13. subst n vala1 vala0.
             match (type of d''eq) with _ = Some (?v) => assert (rel: is_relevant_comp Left (LRC.color (taga v))) by (simpl; auto) end.
             destruct (m_cor_s3 _ _ rel d''eq) as [d' [d'eq d'code]]. simpl in d'eq.
             pose proof (capa_cor1 v14 (n0) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. eauto.
             eapply (unicity (inl _)); eauto. rewrite H17. exact RW0. }
           { destruct (snd (mem_match' w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H13; subst. destruct d''t; try exact H13. destruct H13 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H13. subst n vala1 vala0 comp''.
             pose proof (capa_cor3 (v14 + as_word off0)%w (n0) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. eauto.
             eapply (unicity (inl _)); eauto. rewrite H17. eauto. }
        -- unfold registers_match. simpl. intros w' d.
           split; intro w_eq.
           { repeat (rewrite setmE in w_eq; unfold_match' w_eq; [ convert_eq_op |]; simpl in w_eq).
             all: try simplify_some.
             all: repeat (rewrite setmE; simpl).
             1-11: eexists; split; [| reflexivity].
             1-11: try (split; trivial).
             { rewrite Heqa in Heqa0. unfold evi in Heqa0.
               do 2 unfold_bind; [| simpl in Heqa17; rewrite <- Heqa17 in Heqa21; inversion Heqa21].
               simplify_some. repeat rewrite setmE in Heqa26. simpl in Heqa26. rewrite <- Heqa21 in Heqa26.
               destruct d as [v_d t_d]. unfold is_other in *. destruct t_d; inversion Heqa36.
               split; trivial.
               match (type of Heqa26) with
                 _ = (match ?c with true => Some ?l | false => Some ?r end) =>
                   assert (eq: a44 = if c then l else r) end.
               { goal_match_bind_step; simplify_some; trivial. } subst a44.
               unfold_match' Heqa26; convert_eq_op; simpl.
               ++ simpl in *. repeat rewrite setmE in Heqa25. simpl in Heqa25.
                  rewrite <- Heqa21 in Heqa25. simplify_some.
                  rewrite <- Heqa21 in Heqa17. simplify_some.
                  rewrite <- Heqa21 in RW0. simplify_some. simpl in *.
                  destruct vt_match9 as [? ?]. subst. simpl in *.
                  rewrite H11 in Heqa18. simplify_some.
                  clear - H13. simpl in *.
                  eapply convert_int_of_word_inj; eauto.
               ++ simpl in Heqa17. rewrite <- Heqa17 in Heqa21. simplify_some. destruct a as [v_a ?]. simpl.
                  unfold as_word in Heqa18. simpl in Heqa18. rewrite <- Heqa18 in H11. simplify_some.
                  clear - H13. simpl in *.
                  eapply convert_int_of_word_inj; eauto.
             }
             { simpl. unfold as_word. unfold ssrint.absz. simpl.
               repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end).
               simpl. subst pc3_val. simpl.
               eexists (if (@eq_op (Ord.eqType _) r r0) then _ else _)@Invalidated.
               split.
               2: { simpl. goal_match_bind_step. trivial.
                 goal_match_bind_step. trivial.
                 exfalso.  assert (r_in : In r (domm reg)).
                 { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto. }
                 rewrite reg_domm2 in r_in. simpl in r_in. unfold word_of_nat, as_word in r_in. simpl in r_in.
                 move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
                 repeat (destruct r_in as [|r_in];
                         [subst;
                          (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
                 inversion r_in. }
               split; auto. }
             { exists (Types.vala a46)@Invalidated. split; [split; auto|].
               goal_match_bind_step; auto. convert_eq_op. rewrite setmE in Heqa25. simpl in Heqa25. simplify_some. trivial. }
             { assert (r_in : In w' (domm regs3)).
               { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). exists d. trivial. }
               exfalso. rewrite reg_domm3 in r_in. simpl in r_in. unfold word_of_nat, as_word in r_in. simpl in r_in.
               move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
               repeat (destruct r_in as [|r_in];
                       [subst;
                        (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
               inversion r_in. } }
           { repeat (rewrite setmE in w_eq; unfold_match' w_eq; [ convert_eq_op |]; simpl in w_eq).
             all: try simplify_some.
             all: repeat (rewrite setmE; simpl).
             1-10: eexists; split; [| reflexivity].
             1-10: try (split; trivial).
             { rewrite Heqa in Heqa0. unfold evi in Heqa0.
               do 2 unfold_bind; [| simpl in Heqa17; rewrite <- Heqa17 in Heqa21; inversion Heqa21].
               simplify_some. eexists. split; [|eauto]. destruct a as [va ta]. destruct ta; inversion Heqa36.
               split; auto. repeat rewrite setmE in Heqa26. simpl in Heqa26. rewrite <- Heqa21 in Heqa26.
               unfold_match' Heqa26; repeat simplify_some; convert_eq_op; simpl.
               ++ simpl in *. repeat rewrite setmE in Heqa25. simpl in Heqa25.
                  rewrite <- Heqa21 in Heqa25. simplify_some.
                  rewrite <- Heqa21 in Heqa17. simplify_some.
                  rewrite <- Heqa21 in RW0. simplify_some. simpl in *.
                  destruct vt_match9 as [? ?]. subst. simpl in *.
                  rewrite H11 in Heqa18. simplify_some.
                  clear - H13. simpl in *.
                  eapply convert_int_of_word_inj; eauto.
               ++ simpl in Heqa17. rewrite <- Heqa17 in Heqa21. simplify_some. destruct a11 as [v_a ?]. simpl.
                  unfold as_word in Heqa18. simpl in Heqa18. rewrite <- Heqa18 in H11. simplify_some.
                  clear - H13. simpl in *.
                  eapply convert_int_of_word_inj; eauto.
             }
             { simpl. unfold as_word, ssrint.absz in *. simpl in *.
               repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end).
               simpl. subst pc3_val. simpl.
               (*eexists ((Types.vala a45 + as_word off0)%w @Invalidated). eexists. split; auto. *)
               eexists (if (@eq_op (Ord.eqType _) r0 r) then _ else _)@Invalidated.
               split.
               2: { simpl. goal_match_bind_step. trivial.
                    goal_match_bind_step. trivial.
                    exfalso.  assert (r_in : In r0 (domm reg0)).
                    { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto. }
                    rewrite reg_domm1 in r_in. simpl in r_in. unfold word_of_nat, as_word in r_in. simpl in r_in.
                    move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
                    repeat (destruct r_in as [|r_in];
                            [subst;
                             (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
                    inversion r_in. }
               split; trivial. }
               { simpl. unfold as_word, ssrint.absz in *. simpl in *.
                 repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end).
                 simpl. subst pc3_val. simpl.
                 eexists (if (@eq_op (Ord.eqType _) (as_word (ssrint.Posz 4)) r) then _ else _)@Invalidated. split.
                 2: { unfold as_word. simpl. goal_match_bind_step. trivial. trivial. }
                 split; trivial. }
               {
                 assert (r_in : In w' (domm reg0)).
                 { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto. }
                 exfalso. rewrite reg_domm1 in r_in. simpl in r_in. unfold word_of_nat in r_in. simpl in r_in.
                 move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
                 repeat (destruct r_in as [|r_in];
                         [subst;
                          (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
                 inversion r_in. }
           }
        -- unfold side_of. rewrite ip_comp'. trivial.
        -- unfold register_address_correctness. simpl.
           intros d r1 r_d_eq.
           unfold register_domm in *.
           assert (r_in : In r1 (domm regs3)).
           { clear - reg_domm3 r_d_eq vt_eq10. rewrite reg_domm3.
             repeat (rewrite setmE in r_d_eq; unfold_match' r_d_eq; [ convert_eq_op |]; simpl in r_d_eq).
             all: try (rewrite -Extra.In_in in_fset; red; simpl; tauto).
             (* all: try (simpl; tauto). *)
             rewrite <- reg_domm3. setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto.
             rewrite <- reg_domm3. setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). exists d. trivial. }
           rewrite reg_domm3 in r_in.
           move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
           repeat (destruct r_in as [? | r_in]; try subst r1; repeat (rewrite setmE in r_d_eq; simpl in r_d_eq));
             simpl in r_d_eq; repeat simplify_some; simpl; trivial. unfold_match' r_d_eq.
           { destruct d; unfold is_other in *; unfold_match' Heqa36. }
           inversion r_in.
        -- unfold memory_match. simpl. inversion weak as [? ? ? _ _ m_cor_s2 m_cor_s3 _ _ mem_match']. subst.
           intros w' d d_code d_ip. split; intro w_eq.
           { destruct (fst (mem_match w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H13; subst. destruct d''t; try exact H13. destruct H13 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H13. subst n vala1 vala0 comp'.
             match (type of d''eq) with _ = Some (?v) => assert (rel: is_relevant_comp Right (LRC.color (taga v))) by (simpl; auto) end.
             destruct (s_mem_cor _ _ rel d''eq) as [d' [d'eq d'code]]. simpl in d'eq.
             pose proof (capa_cor2 w (n0) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. rewrite <- H17. eauto.
             eapply (unicity (inl _)); eauto. rewrite H17. exact RW. }
           { destruct (snd (mem_match w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H13; subst. destruct d''t; try exact H13. destruct H13 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H13. subst n vala1 vala0 comp''.
             match (type of d''eq) with _ = Some (?v) => assert (rel: is_relevant_comp Right (LRC.color (taga v))) by (simpl; auto) end.
             subst. destruct (s'_mem_cor _ _ rel d''eq) as [d' [d'eq d'code]]. simpl in d'eq.
             pose proof (capa_cor3 (v14 + as_word off0)%w (n0) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. rewrite <- H17. eauto.
             eapply (unicity (inl _)); eauto. rewrite H17. eauto. }
    + (* return from comp in C to comp' in C *)
      repeat
      (match goal with
         H : Some ?a = getm reg _ |- _ => deduce_equality (esym H); pose proof (esym H); clear H
       end).
      deduce_equality RW.
      deduce_equality RA.
      subst s2. unfold side_of in *. unfold_match' side_eq. simpl in *.
      assert (side_eq': true = (comp \in domm ic)).
      { assert (comp_incl: comp \in (domm ic) \/ comp \in (domm ip)) by
          (rewrite ST0 in col_mem1; pose proof (col_mem1 _ _ PC0) as in_disj; simpl in in_disj;
           setoid_rewrite <- Extra.In_in in in_disj; exact ((fst (or_comm _ _)) in_disj)).
        destruct comp_incl as [Hl | Hr]. rewrite Hl. trivial. rewrite Hr in Heqa5. inversion Heqa5. }
      deduce_equality PC.
      assert (Meq: exists top M', M = top :: M').
      { destruct vt_match10 as [ret_eq stack_prop]. unfold color_of in stack_prop.
        subst t10. simpl in stack_prop. destruct stack_prop as [top [col top_in]].
        destruct M; eauto. inversion top_in. }
      destruct Meq as [top [M' M_eq]].
      destruct vt_match10 as [ret_eq stack_prop].
      subst t10. simpl in stack_prop. destruct stack_prop as [s2val [comp'' triple_in]].
      match (type of triple_in) with (_ ?t _) => assert (top = t); [|subst top] end.
      { inv tag_pc1. simpl in *. subst.
        clear- wfst triple_in. destruct triple_in as [?| is_in]; auto. exfalso.
        remember (Nat.pred n) as n'. assert (n'_ineq: S n' >= n) by (subst; clear; induction n; eauto).
        clear Heqn'. revert is_in. revert n'_ineq. revert w s2val v11 comp'' n'.
        inversion wfst; subst; clear - WF_ST; simpl; unfold Init.Nat.pred, "+", addn_rec; rewrite Nat.add_1_r.
        all: induction WF_ST; intros a v b col n' n'_ineq is_in; inversion is_in.
        all: try (inversion H; subst n; (assert false; [|done]); rewrite <- (ltn_subrR 0 (S n')); unfold subn, subn_rec, Nat.sub;
                  unfold addn, addn_rec in n'_ineq; rewrite Nat.add_1_r in n'_ineq; done).
        all: eapply (IHWF_ST _ _ _ _ _ _ H). }
      assert (comp'' = comp'); [|subst comp''].
      { unfold_match' Heqa2. inversion Heqa2 as [tag_mem_w].
        clear - wfst M_eq Heqa6 tag_mem_w. destruct a10. simpl in *. subst taga.
        inversion wfst; subst M; inv H0; try rename PTS_COD22 into PTS_CODE2;
        unfold points_to_comp_code in PTS_CODE2; oapp_False; simpl in *; rewrite <- Heqa6 in HeqH; simplify_some;
        destruct PTS_CODE2 as [col ?]; simpl in col; auto. }
      inversion wfst; subst M; try (inv H0; done); simpl in *.
      1: { exfalso. inv H13. clear - SIDE ic_comp' Hmergeable_ifaces.
           eapply (@Machine.Intermediate.fdisjoint_partition_notinboth _ (domm ip) (domm ic)); eauto.
           inversion Hmergeable_ifaces as [[_ fdisj] _]; eauto. }
      inversion H13. subst n s2val v15 v16 C st. clear H19 H13.
      unfold points_to_comp_code in PTS_CODE3. oapp_False.
      eexists. exists M'.
      split.
      * eapply (plus_left _ [:: ERet comp rcom_value1 comp'] ). try eapply step_jump; eauto.
        -- rewrite eq_s3 in vt_eq12. exact vt_eq12.
        -- inversion vt_match12; subst; revert H12; try decode_instr_eq.
        -- rewrite eq_s3 in vt_eq10. exact vt_eq10.
        -- rewrite eq_s3 in vt_eq11. exact vt_eq11.
        -- unfold reg_clear_list, reg_list. simpl. unfold as_word. subst. simpl.
           repeat
             (match goal with
              | H: _ ?w = _  |- context[_ ?w] => rewrite H; simpl
              end). trivial.
        -- unfold next_state_updates, next_state_updates_and_pc, next_state, transfer, instr_rules, LRC.instr_rules in *.
           unfold evi in *. simpl. unfold_bind.
           deduce_reg reg_match. simpl in *. rewrite <- H18.
           rewrite <- HeqH12. simpl. rewrite eq_s3 in tag_pc3. simpl in tag_pc3.
           rewrite tag_pc3. rewrite eq_refl. simpl.
           destruct PTS_CODE3 as [col_a10 code_a10]. destruct a10 as [va10 ta10]. destruct ta10.
           simpl in col_a10. rewrite col_a10. simpl. rewrite eq_refl. simpl. rewrite <- Heqa35. simpl.
           destruct a as [? ta]. unfold is_other in *. unfold_match' Heqa36. simpl in *.
           subst ta. inversion vt_match9 as [? _]. subst t9. simpl.
           unfold updm. unfold as_word. simpl in *.
           (*automatically rewrite regs3 registers*)
           repeat
             (repeat
                (match goal with
                 | H: getm ?r ?w = _  |- context[getm ?r ?w] => rewrite H; simpl
                 end);
              (*automatically transforms setm in if (_ == _) then _ else _*)
              repeat rewrite setmE; simpl;
              (*destruct the (_ == _) condition *)
              try
                (let cond := fresh "cond" in
                 match goal with
                 | |- context[@eq_op ?t ?a ?r] =>
                     remember (@eq_op t a r) as cond; destruct cond;
                     [convert_eq_op; simpl in *|]; simpl
                 end)).
           all: inversion Heqa0; subst rcom_value1; rewrite ST0 in Heqa11; simpl in Heqa11; rewrite H11 in Heqa;
             simpl in Heqa; inversion Heqa; rewrite H14; destruct x, H12; subst; simpl.
           13: reflexivity. (* most general case, to be treated first *)
           all: (rewrite (setmxx _ _ (v12@Invalidated) _ )) || (rewrite (setmC _ (v12@Invalidated)); [|done]).
           all: unfold as_word in *; simpl in *.
           all: (match (type of vt_eq10) with
                 | ?ls = ?rs =>
                     revert vt_eq10;
                     (match goal with
                        Heq: ls = Some (?a) |- _ =>
                          intro vt_eq10; rewrite Heq in vt_eq10; simplify_some; subst; simpl
                      end)
                 end).
           all: repeat (rewrite setmxx || (rewrite (setmC _ ((addw _ _)@_)); [|done]) ).
           all: trivial.
           repeat ((rewrite (setmC _ (v12@_))); [|done]). simplify_some. trivial.
        -- eapply star_refl.
        -- reflexivity.
      * rewrite ST0 in tag_pc1. simpl in tag_pc1. inversion tag_pc1. subst n1. clear tag_pc1.
        rewrite eq_s3 in tag_pc3. simpl in tag_pc3. subst pc3_tag. subst. simpl in *.
        repeat rewrite <- H17.
        eapply match_states_right; econstructor; simpl; unfold build_tpc; try trivial.
        -- unfold register_domm. repeat rewrite domm_set. simpl.
           unfold register_domm, reg_field_size, mword, FSet.fsval in reg_domm1. simpl in reg_domm1.
           unfold mword, word_size. simpl.
           (* clear - reg_domm1 RW0. admit. (* exstructure *) *)
           clear -reg_domm1 RW0.
           rewrite //= reg_domm1 //=.
           repeat match goal with
             | |- context [as_word (ssrint.Posz ?n)] =>
                 replace (@as_word 5 (ssrint.Posz n)) with
                 (@word_of_nat 5 n) by reflexivity
           end.
           rewrite <- eq_fset.
           intros r. rewrite in_fset.
           match goal with
           | |- (r \in (word_of_nat _ |: _)%fset) = (r \in _) =>
               rewrite in_fset in_cons
           end;
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=| clear eqr]
           end.
           repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
           rewrite in_fset in_cons.
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
           end.
           ++ by rewrite -in_fset -reg_domm1 mem_domm RW0.
           ++ repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
              now rewrite in_fset.
        -- unfold register_domm. repeat rewrite domm_set. simpl.
           unfold register_domm, reg_field_size, mword, FSet.fsval in reg_domm2. simpl in reg_domm2.
           unfold mword, word_size. simpl.
           clear -reg_domm2 RW.
           rewrite //= reg_domm2 //=.
           repeat match goal with
             | |- context [as_word (ssrint.Posz ?n)] =>
                 replace (@as_word 5 (ssrint.Posz n)) with
                 (@word_of_nat 5 n) by reflexivity
           end.
           rewrite <- eq_fset. rename r into r0.
           intros r. rewrite in_fset.
           match goal with
           | |- (r \in (word_of_nat _ |: _)%fset) = (r \in _) =>
               rewrite in_fset in_cons
           end;
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=| clear eqr]
           end.
           repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
           rewrite in_fset in_cons.
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
           end.
           ++ by rewrite -in_fset -reg_domm2 mem_domm RW.
           ++ repeat (match goal with
                   | |- (r \in FSet.fsval (word_of_nat _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
              now rewrite in_fset.
        -- unfold register_domm. repeat rewrite domm_set. simpl.
           unfold register_domm, reg_field_size, mword, FSet.fsval in reg_domm3. simpl in reg_domm3.
           unfold mword, word_size. simpl.
           clear - reg_domm3 vt_eq10.
           rewrite //= reg_domm3 //=.
           repeat match goal with
             | |- context [as_word (ssrint.Posz ?n)] =>
                 replace (@as_word 5 (ssrint.Posz n)) with
                 (@word_of_nat 5 n) by reflexivity
           end.
           unfold word_of_nat. unfold as_word. simpl.
           rewrite <- eq_fset. rename r into r0.
           intros r. rewrite in_fset.
           match goal with
           | |- (r \in (Word _ |: _)%fset) = (r \in _) =>
               rewrite in_fset in_cons
           end;
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=| clear eqr]
           end.
           repeat (match goal with
                   | |- (r \in FSet.fsval (Word _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
           rewrite in_fset in_cons.
           match goal with
           | |- (r == ?n) || ?b = (r \in ?l) =>
               let eqr := fresh "eqr" in
               case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
           end.
           ++ by rewrite -in_fset -reg_domm3 mem_domm vt_eq10.
           ++ repeat (match goal with
                   | |- (r \in FSet.fsval (Word _ |: _)%fset) = (r \in _) =>
                       rewrite in_fset in_cons
                   end;
                   match goal with
                   | |- (r == ?n) || ?b = (r \in ?l) =>
                       let eqr := fresh "eqr" in
                       case eqr: eq_op => //=; [move: eqr => /eqP -> //=|clear eqr]
                   end).
              now rewrite in_fset.
        -- split; [|split; [|split; [|split; [|split; [|split; [| split]]]]]]; trivial.
        -- split; [|split; [|split; [|split; [|split; [|split; [| split]]]]]]; trivial.
        -- split; [|split; [|split; [|split; [|split; [|split; [| split]]]]]]; trivial.
        -- intros v' n rw rweq. simpl in *. destruct rw as [r' | w'].
           { exfalso. repeat rewrite setmE in rweq. repeat unfold_match' rweq.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list))
               by (rewrite -in_fset -reg_domm1; rewrite mem_domm; rewrite rweq; trivial).
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in. }
           pose proof (capa_cor1 v' n (inr w') rweq) as [is_in unicity]. simpl in unicity. split.
           { unfold in_stack in *. destruct is_in as [sv1 [sv2 [col [|is_in]]]]; [|eauto].
             exfalso. inversion H12. subst n v' sv1 sv2 col. eapply (unicity (inl _)). rewrite H17. exact RW0. }
           intros rw v''. destruct rw as [r' | w'']; simpl.
           { intro rweq'. exfalso. repeat rewrite setmE in rweq'. repeat unfold_match' rweq'.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list))
               by (rewrite -in_fset -reg_domm1; rewrite mem_domm; rewrite rweq'; trivial).
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in.  }
           intros [t' [v''eq t'eq]]. eapply (unicity (inr _)); eauto.
        -- intros v' n rw rweq. simpl in *. destruct rw as [r' | w'].
           { exfalso. repeat rewrite setmE in rweq. repeat unfold_match' rweq.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list))
               by (rewrite -in_fset -reg_domm2; rewrite mem_domm; rewrite rweq; trivial).
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in. }
           pose proof (capa_cor2 v' n (inr w') rweq) as [is_in unicity]. simpl in unicity. split.
           { unfold in_stack in *. destruct is_in as [sv1 [sv2 [col [|is_in]]]]; [|eauto].
             exfalso. inversion H12. subst n v' sv1 sv2 col. eapply (unicity (inl _)). exact RW. }
           intros rw v''. destruct rw as [r' | w'']; simpl.
           { intro rweq'. exfalso. repeat rewrite setmE in rweq'. repeat unfold_match' rweq'.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list))
               by (rewrite -in_fset -reg_domm2; rewrite mem_domm; rewrite rweq'; trivial).
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold word_of_nat in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in.  }
           intros [t' [v''eq t'eq]]. eapply (unicity (inr _)); eauto.
        -- intros v' n rw rweq. simpl in *. destruct rw as [r' | w'].
           { exfalso. repeat rewrite setmE in rweq. repeat unfold_match' rweq.
             repeat simplify_some. inversion Heqa36.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list))
               by (rewrite -in_fset -reg_domm3; rewrite mem_domm ; rewrite rweq; trivial).
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. unfold as_word in r'_in. simpl in r'_in.
             unfold word_of_nat, as_word in r'_in. simpl in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in. }
           pose proof (capa_cor3 v' n (inr w') rweq) as [is_in unicity]. simpl in unicity. split.
           { unfold in_stack in *. destruct is_in as [sv1 [sv2 [col [|is_in]]]]; [|eauto].
             exfalso. inversion H12. subst n v' sv1 sv2 col. eapply (unicity (inl _)). exact vt_eq10. }
           intros rw v''. destruct rw as [r' | w'']; simpl.
           { intro rweq'. exfalso. repeat rewrite setmE in rweq'. repeat unfold_match' rweq'.
             repeat simplify_some. inversion Heqa36.
             assert (r'_in : r' \in ra :: (map (word_of_nat) reg_list))
               by (rewrite -in_fset -reg_domm3; rewrite mem_domm; rewrite rweq'; trivial).
             simpl in r'_in. setoid_rewrite (Extra.In_in) in r'_in. simpl in r'_in. unfold word_of_nat, as_word in r'_in. simpl in r'_in.
             repeat (destruct r'_in as [|r'_in];
                     [subst;
                      (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
             inversion r'_in.  }
           intros [t' [v''eq t'eq]]. eapply (unicity (inr _)); eauto.
        (* -- inversion weak; trivial. *)
        -- unfold side_of. goal_match_bind_step; trivial.
           exfalso. eapply (@Machine.Intermediate.fdisjoint_partition_notinboth _ (domm ip) (domm ic)); eauto.
           inversion Hmergeable_ifaces as [[_ fdisj] _]; eauto.
        -- inversion weak; trivial.
        -- inversion weak; trivial.
        -- inversion weak; trivial.
        -- unfold register_address_correctness. simpl.
           intros d r1 r_d_eq.
           unfold register_domm in *.
           assert (r_in : In r1 (domm regs3)).
           { clear - reg_domm3 r_d_eq vt_eq10. rewrite reg_domm3.
             repeat (rewrite setmE in r_d_eq; unfold_match' r_d_eq; [ convert_eq_op |]; simpl in r_d_eq).
             all: try (rewrite -Extra.In_in in_fset; red; simpl; tauto).
             rewrite <- reg_domm3. setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto.
             rewrite <- reg_domm3. setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). exists d. trivial. }
           rewrite reg_domm3 in r_in.
           move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
           repeat (destruct r_in as [? | r_in]; try subst r1; repeat (rewrite setmE in r_d_eq; simpl in r_d_eq));
             simpl in r_d_eq; repeat simplify_some; simpl; trivial. unfold_match' r_d_eq.
           { destruct d; unfold is_other in *; unfold_match' Heqa36. }
           inversion r_in.
        -- unfold memory_match. simpl. inversion weak as [? ? ? _ _ m_cor_s3 m_cor_s2 _ _ mem_match']. subst.
           intros w' d d_code d_ip.
           split; intro w_eq.
           { destruct (fst (mem_match' w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H13; subst. destruct d''t; try exact H13. destruct H13 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H13. subst n vala1 vala0.
             match (type of d''eq) with _ = Some (?v) => assert (rel: is_relevant_comp Left (LRC.color (taga v))) by (simpl; auto) end.
             destruct (m_cor_s3 _ _ rel d''eq) as [d' [d'eq d'code]]. simpl in d'eq.
             pose proof (capa_cor1 v14 (n0) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. eauto.
             eapply (unicity (inl _)); eauto. rewrite H17. exact RW0. }
           { destruct (snd (mem_match' w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H13; subst. destruct d''t; try exact H13. destruct H13 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H13. subst n vala1 vala0 comp''.
             pose proof (capa_cor3 (w + as_word off0)%w (n0) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. eauto.
             eapply (unicity (inl _)); eauto. rewrite H17. eauto. }
        -- unfold_match' Heqa2. rewrite <- H17 in RW0.
           pose proof (capa_cor1 w0 (n0) (inl r0) RW0) as [is_in unicity].
           unfold in_stack in is_in. simpl in is_in.
           eapply same_pc_normal; simpl; eauto.
        -- unfold side_of. goal_match_bind_step; trivial.
           exfalso. eapply (@Machine.Intermediate.fdisjoint_partition_notinboth _ (domm ip) (domm ic)); eauto.
           inversion Hmergeable_ifaces as [[_ fdisj] _]; eauto.
        -- unfold register_address_correctness. simpl.
           intros d r1 r_d_eq. repeat rewrite setmE in r_d_eq. repeat unfold_match' r_d_eq.
           unfold register_domm in *.
           assert (r_in : In r1 (domm reg)).
           { clear - reg_domm2 r_d_eq. setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto. }
           rewrite reg_domm2 in r_in. unfold word_of_nat in r_in. simpl in r_in.
           move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
           repeat (destruct r_in as [|r_in];
                   [subst;
                    (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
           inversion r_in.
        -- unfold register_address_correctness. simpl.
           intros d r1 r_d_eq.
           unfold register_domm in *.
           assert (r_in : In r1 (domm regs3)).
           { clear - reg_domm3 r_d_eq vt_eq10. rewrite reg_domm3.
             repeat (rewrite setmE in r_d_eq; unfold_match' r_d_eq; [ convert_eq_op |]; simpl in r_d_eq).
             all: try (rewrite -Extra.In_in in_fset; red; simpl; tauto).
             rewrite <- reg_domm3. setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto.
             rewrite <- reg_domm3. setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). exists d. trivial. }
           rewrite reg_domm3 in r_in.
           move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
           repeat (destruct r_in as [? | r_in]; try subst r1; repeat (rewrite setmE in r_d_eq; simpl in r_d_eq));
             simpl in r_d_eq; repeat simplify_some; simpl; trivial. unfold_match' r_d_eq.
           { destruct d; unfold is_other in *; unfold_match' Heqa36. }
           inversion r_in.
        -- unfold memory_match. simpl. inversion weak as [? ? ? _ _ m_cor_s2 m_cor_s3 _ _ mem_match']. subst.
           intros w' d d_code d_ip. split; intro w_eq.
           { destruct (fst (mem_match w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H13; subst. destruct d''t; try exact H13. destruct H13 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H13. subst n vala1 vala0 comp'.
             match (type of d''eq) with _ = Some (?v) => assert (rel: is_relevant_comp Right (LRC.color (taga v))) by (simpl; auto) end.
             destruct (s_mem_cor _ _ rel d''eq) as [d' [d'eq d'code]]. simpl in d'eq.
             pose proof (capa_cor2 w (n0) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. rewrite <- H17. eauto.
             eapply (unicity (inl _)); eauto. rewrite H17. exact RW. }
           { destruct (snd (mem_match w' d d_code d_ip) w_eq) as [d'' [d''match d''eq]].
             simpl in *. exists d''. split; [|exact d''eq].
             destruct d; destruct d'' as [? d''t]; destruct d''t as [d''t ? ? ?]. destruct d''match; subst;
               split; [trivial|]; split; [trivial|]; simpl in *.
             destruct H13; subst. destruct d''t; try exact H13. destruct H13 as [sv' [comp'' inM]].
             destruct inM ; [|exists sv', comp''; trivial].
             exfalso. inversion H13. subst n vala1 vala0 comp''.
             match (type of d''eq) with _ = Some (?v) => assert (rel: is_relevant_comp Right (LRC.color (taga v))) by (simpl; auto) end.
             subst. destruct (s'_mem_cor _ _ rel d''eq) as [d' [d'eq d'code]]. simpl in d'eq.
             pose proof (capa_cor3 (w + as_word off0)%w (n0) (inr w')) as cor. simpl in cor.
             destruct cor as [is_in unicity]. eexists. split. exact d''eq. rewrite <- H17. eauto.
             eapply (unicity (inl _)); eauto. rewrite H17. eauto. }
        -- unfold registers_match. simpl. intros w' d.
           split; intro w_eq.
           { repeat (rewrite setmE in w_eq; unfold_match' w_eq; [ convert_eq_op |]; simpl in w_eq).
             all: try simplify_some.
             all: repeat (rewrite setmE; simpl).
             1-11: eexists; split; [| reflexivity].
             1-11: try (split; trivial).
        (*      { destruct d as [v_d t_d]. unfold is_other in *. destruct t_d; inversion Heqa51. *)
        (*        split; trivial. rewrite setmE in Heqa41. *)
        (*        unfold_match' Heqa41; simplify_some; trivial. simpl. convert_eq_op. *)
        (*        rewrite setmE in Heqa40. unfold_match' Heqa40. rewrite <- Heqa40 in H10. simplify_some. trivial. *)
        (*        rewrite setmE in Heqa41. unfold_match' Heqa41. rewrite <- Heqa41 in H10. simplify_some. trivial. } *)
        (*      { simpl. unfold as_word. unfold ssrint.absz. simpl. *)
        (*        repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end). *)
        (*        simpl. subst pc3_val. simpl. eexists ((Types.vala a45)@Invalidated). eexists. split; auto. *)
        (*        rewrite eq_refl. trivial. } *)
        (*      { rewrite <- Heqa31. eexists (_ @ Invalidated). split; eauto. split; auto. } *)
        (*      { assert (r_in : In w' (domm regs3)). *)
        (*        { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). exists d. trivial. } *)
        (*        exfalso. rewrite reg_domm3 in r_in. simpl in r_in. unfold word_of_nat, as_word in r_in. simpl in r_in. *)
        (*        move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in. *)
        (*        repeat (destruct r_in as [|r_in]; *)
        (*                [subst; *)
        (*                 (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]). *)
        (*        inversion r_in. } } *)
        (*    { repeat (rewrite setmE in w_eq; unfold_match' w_eq; [ convert_eq_op |]; simpl in w_eq). *)
        (*      all: try simplify_some. *)
        (*      all: repeat (rewrite setmE; simpl). *)
        (*      1-10: eexists; split; [| reflexivity]. *)
        (*      1-10: try (split; trivial). *)
        (*      { destruct a23 as [v_d t_d]. unfold is_other in *. destruct t_d; inversion Heqa51. *)
        (*        eexists. split; trivial. rewrite setmE in Heqa41. *)
        (*        unfold_match' Heqa41; simplify_some; trivial. simpl. convert_eq_op. *)
        (*        rewrite setmE in Heqa40. unfold_match' Heqa40. rewrite <- Heqa40 in H10. simplify_some. split; trivial. *)
        (*        rewrite setmE in Heqa41. unfold_match' Heqa41. rewrite <- Heqa41 in H10. simplify_some. split; trivial. } *)
        (*      { simpl. unfold as_word, ssrint.absz in *. simpl in *. *)
        (*        repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end). *)
        (*        simpl. subst pc3_val. simpl. eexists ((Types.vala a45 + as_word off0)%w @Invalidated). eexists. split; auto. *)
        (*        rewrite eq_refl. repeat rewrite setmE in Heqa40. repeat unfold_match' Heqa40; convert_eq_op. *)
        (*        simplify_some. rewrite <- Heqa39 in RW0. simplify_some. reflexivity. *)
        (*        rewrite <- Heqa40 in RW0. simplify_some. reflexivity. } *)
        (*      { simpl. unfold as_word, ssrint.absz in *. simpl in *. *)
        (*        repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end). *)
        (*        simpl. subst pc3_val. simpl. eexists. split; auto. split; auto. } *)
        (*      assert (r_in : In w' (domm reg0)). *)
        (*      { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). exists d. trivial. } *)
        (*      exfalso. rewrite reg_domm1 in r_in. simpl in r_in. unfold word_of_nat in r_in. simpl in r_in. *)
        (*        move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in. *)
        (*       repeat (destruct r_in as [|r_in]; *)
        (*              [subst; *)
        (*               (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]). *)
        (*       inversion r_in. } *)
        (* -- unfold registers_match. simpl. intros w' d. *)
        (*    split; intro w_eq. *)
        (*    { repeat (rewrite setmE in w_eq; unfold_match' w_eq; *)
        (*              [ convert_eq_op |]; simpl in w_eq). *)
        (*      all: try simplify_some. *)
        (*      all: repeat (rewrite setmE; simpl). *)
        (*      1-11: eexists; split; [| reflexivity]. *)
        (*      1-11: try (split; trivial). *)
             { rewrite Heqa in Heqa0. unfold evi in Heqa0.
               do 2 unfold_bind; [| simpl in Heqa17; rewrite <- Heqa17 in Heqa21; inversion Heqa21].
               simplify_some. repeat rewrite setmE in Heqa26. simpl in Heqa26. rewrite <- Heqa21 in Heqa26.
               destruct d as [v_d t_d]. unfold is_other in *. destruct t_d; inversion Heqa36.
               split; trivial.
               simpl in Heqa17.
               match (type of Heqa26) with
                 _ = (match ?c with true => Some ?l | false => Some ?r end) =>
                   assert (eq: a44 = if c then l else r) end.
               { goal_match_bind_step; simplify_some; trivial. } subst a44.
               unfold_match' Heqa26; convert_eq_op; simpl.
               ++ simpl in *. repeat rewrite setmE in Heqa25. simpl in Heqa25.
                  (* assert (a23 = a) by congruence. subst a23. *)
                  rewrite <- Heqa21 in Heqa25. simplify_some.
                  (* assert (a11 = a) by congruence. subst a11. *)
                  rewrite <- Heqa21 in Heqa17. simplify_some.
                  rewrite <- Heqa21 in RW0. simplify_some. simpl in *.
                  destruct vt_match9 as [? ?]. subst. simpl in *.
                  rewrite H11 in Heqa18. simplify_some.
                  destruct eq_op eqn:eq_r; eauto.
                  move: eq_r => /eqP eq_r.
                  rewrite -eq_r in a21_eq.
                  simpl in a21_eq.
                  rewrite H11 in a21_eq. inv a21_eq. auto.
               ++ simpl in Heqa17. rewrite <- Heqa17 in Heqa21. simplify_some. destruct a as [v_a ?]. simpl.
                  unfold as_word in Heqa18. simpl in Heqa18. rewrite <- Heqa18 in H11. simplify_some.
                  destruct eq_op eqn:eq_r; eauto.
                  move: eq_r => /eqP eq_r.
                  rewrite -eq_r in a21_eq.
                  simpl in a21_eq.
                  rewrite -Heqa18 in a21_eq. inv a21_eq. eauto.
             }
             { simpl. unfold as_word. unfold ssrint.absz. simpl.
               repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end).
               simpl. subst pc3_val. simpl. rewrite eq_refl. simpl. eexists; split; [|eauto]. split; trivial. }
             { rewrite <- Heqa18. simpl. eexists; split; [|eauto]. split; auto. }
             { assert (r_in : In w' (domm regs3)).
               { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). exists d. trivial. }
               exfalso. rewrite reg_domm3 in r_in. simpl in r_in. unfold word_of_nat, as_word in r_in. simpl in r_in.
               move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
               repeat (destruct r_in as [|r_in];
                       [subst;
                        (match goal with H : false = (Word ?a == Word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
               inversion r_in. } }
           { repeat (rewrite setmE in w_eq; unfold_match' w_eq; [ convert_eq_op |]; simpl in w_eq).
             all: try simplify_some.
             all: repeat (rewrite setmE; simpl).
             1-10: eexists; split; [| reflexivity].
             1-10: try (split; trivial).
             { rewrite Heqa in Heqa0. unfold evi in Heqa0.
               do 2 unfold_bind; [| simpl in Heqa17; rewrite <- Heqa17 in Heqa21; inversion Heqa21].
               simplify_some. eexists. split; [|eauto]. destruct a as [va ta]. destruct ta; inversion Heqa36.
               split; auto. repeat rewrite setmE in Heqa26. simpl in Heqa26. rewrite <- Heqa21 in Heqa26.
               unfold_match' Heqa26; repeat simplify_some; convert_eq_op; simpl.
               ++ simpl in *. repeat rewrite setmE in Heqa25. simpl in Heqa25.
                  rewrite <- Heqa21 in Heqa25. simplify_some.
                  rewrite <- Heqa21 in Heqa17. simplify_some.
                  rewrite <- Heqa21 in RW0. simplify_some. simpl in *.
                  destruct vt_match9 as [? ?]. subst. simpl in *.
                  rewrite H11 in Heqa18. simplify_some.
                  destruct eq_op eqn:eq_r; eauto.
                  move: eq_r => /eqP eq_r.
                  rewrite -eq_r in a21_eq.
                  simpl in a21_eq.
                  rewrite H11 in a21_eq. inv a21_eq. auto.
               ++ simpl in Heqa17. rewrite <- Heqa17 in Heqa21. simplify_some. destruct a11 as [v_a ?]. simpl.
                  unfold as_word in Heqa18. simpl in Heqa18. rewrite <- Heqa18 in H11. simplify_some.
                  destruct eq_op eqn:eq_r; eauto.
                  move: eq_r => /eqP eq_r.
                  rewrite -eq_r in a21_eq.
                  simpl in a21_eq.
                  rewrite -Heqa18 in a21_eq. inv a21_eq. eauto.
             }
             { simpl. unfold as_word, ssrint.absz in *. simpl in *.
               repeat (match goal with | H: false = ?c |- context[?c] => rewrite <- H end).
               simpl. subst pc3_val. simpl. rewrite eq_refl. eexists; split; [|eauto]. split; trivial. }
             { rewrite <- Heqa18. simpl. eexists; split; [|eauto]. split; auto. }
             { assert (r_in : In w' (domm reg)).
               { setoid_rewrite <- Extra.In_in. setoid_rewrite <- (rwP dommP). eauto. }
               exfalso. rewrite reg_domm2 in r_in. simpl in r_in. unfold word_of_nat in r_in. simpl in r_in.
               move: r_in; rewrite -Extra.In_in in_fset Extra.In_in => r_in.
               repeat (destruct r_in as [|r_in];
                       [subst;
                        (match goal with H : false = (as_word ?a == as_word ?a) |- _ => rewrite eq_refl in H; inversion H; done end) |]).
               inversion r_in. }
           }
  Admitted.
  
End StepEventReturn.
