...
(Remove whatever is not in previous sections, hopefully empty)
...
Source.well_formed_program_unlink
  : forall (Cs : {fset Component.id}) (p : Source.program),
    Source.well_formed_program p ->
    Source.well_formed_program (Source.program_unlink Cs p)
Definability.well_formed_events_well_formed_program
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    (forall (C : nat_ordType) (buf : nat + list value),
     prog_buffers C = Some buf -> Buffer.well_formed_buffer buf) ->
    forall (T : Type) (procs : NMap (NMap T)) (t : list event_inform),
    seq.all (well_formed_event intf procs) t ->
    Source.well_formed_program
      (Definability.program_of_trace intf prog_buffers t)
well_formed_compilable
  : forall p : Source.program,
    Source.well_formed_program p ->
    exists pc : Intermediate.program, compile_program p = Some pc
star_well_formed_intermediate_prefix
  : forall (p : Intermediate.program) (t : Events.trace event_inform)
      (s : state (I.CS.sem_inform p)),
    Intermediate.well_formed_program p ->
    Star (I.CS.sem_inform p) (I.CS.initial_machine_state p) t s ->
    Definability.well_formed_intermediate_prefix
      (Intermediate.prog_interface p) (Intermediate.prog_buffers p) t
...
Definability.pointer_of_alloc
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    domm (T:=nat_ordType) (S:=Component.interface) intf =
    domm (T:=nat_ordType) (S:=nat + list value) prog_buffers ->
    (forall (C : nat_ordType) (buf : nat + list value),
     prog_buffers C = Some buf -> Buffer.well_formed_buffer buf) ->
    forall (t : Events.trace event_inform) (T : Type)
      (t_procs : NMap (NMap T)),
    domm (T:=nat_ordType) (S:=NMap T) t_procs =
    domm (T:=nat_ordType) (S:=Component.interface) intf ->
    seq.all (well_formed_event intf t_procs) t ->
    forall p_interm : Intermediate.program,
    (exists s : CS.state,
       CSInvariants.CSInvariants.is_prefix s p_interm (project_non_inform t)) ->
    forall (mem : eqtype.Equality.sort Memory.Memory.t) 
      (cid : Component.id) (sz : nat)
      (mem' : eqtype.Equality.sort Memory.Memory.t) 
      (ptr' : Pointer.t) (nb : Block.id),
    Memory.Memory.alloc mem cid sz = Some (mem', ptr') ->
    Definability.next_block mem cid = Some nb ->
    ptr' = (Permission.data, cid, nb, 0%Z)
Compiler.order : Compiler.index -> Compiler.index -> Prop
CSInvariants.CSInvariants.not_executing_can_not_share
  : forall (s : CS.state) (p : Intermediate.program) 
      (t : list Events.event) (e : Events.event) (C : Component.id)
      (b : Block.id),
    Intermediate.well_formed_program p ->
    Intermediate.closed_program p ->
    CSInvariants.CSInvariants.is_prefix s p (seq.rcons t e) ->
    C <> Events.cur_comp_of_event e ->
    (forall b' : Block.id, ~ addr_shared_so_far (C, b') t) ->
    ~ addr_shared_so_far (C, b) (seq.rcons t e)
Definability.next_block_prepare_buffers
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    forall C : nat_ordType,
    Definability.component_buffer intf C ->
    Definability.next_block
      (Source.prepare_buffers
         (Definability.program_of_trace intf prog_buffers t)) C =
    Some Definability.LOCALBUF_blockid
Definability.next_block_initial_memory
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    Definability.component_buffer intf C ->
    Definability.next_block
      (mkfmapf (T:=nat_ordType)
         (fun C0 : nat_ordType =>
          Memory.ComponentMemory.prealloc
            match prog_buffers C0 with
            | Some buf => [fmap (Block.local, buf)]
            | None => emptym
            end) (domm (T:=nat_ordType) (S:=Component.interface) intf)) C =
    Some 1
Definability.next_block_alloc_neq
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    (exists s : CS.state,
       CSInvariants.CSInvariants.is_prefix s p_interm (project_non_inform t)) ->
    forall (m : eqtype.Equality.sort Memory.Memory.t) 
      (C : Component.id) (n : nat)
      (m' : eqtype.Equality.sort Memory.Memory.t) 
      (b : Pointer.t) (C' : Component.id),
    Memory.Memory.alloc m C n = Some (m', b) ->
    C' <> C -> Definability.next_block m' C' = Definability.next_block m C'
Definability.next_block_alloc
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    forall (m : eqtype.Equality.sort Memory.Memory.t) 
      ...
      (b : Pointer.t),
    Memory.Memory.alloc m C n = Some (m', b) ->
    Definability.next_block m C = Some (Pointer.block b) /\
    Definability.next_block m' C = Some (ssrnat.addn (Pointer.block b) 1)
Compiler.match_states
  : forall (p : Source.program) (tp : Intermediate.program),
    Compiler.index ->
    state (S.CS.sem p) -> state (I.CS.sem_non_inform tp) -> Prop
Definability.load_prepare_buffers
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    Memory.Memory.load
      (Source.prepare_buffers
         (Definability.program_of_trace intf prog_buffers t))
      (Permission.data, C, Block.local, Z.of_nat o) =
    nth_error Definability.meta_buffer o
Definability.load_next_block_None
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    Intermediate.prog_interface p_interm = intf ->
    forall (mem : eqtype.Equality.sort Memory.Memory.t) 
      (ptr : Pointer.t) (b : Block.id),
    Definability.next_block mem (Pointer.component ptr) = Some b ->
    Pointer.block ptr >= b -> Memory.Memory.load mem ptr = None
Memory.ComponentMemory.load_next_block
CS.load_data_next_block
  : forall p : Source.program,
    Source.well_formed_program p ->
    Source.closed_program p ->
    forall (s : Source.CS.CS.state) (t : Events.trace Events.event)
      (s' : state (CS.sem p)) (ptr : Pointer.t) (C : Component.id)
      (b : Block.id) (o : Block.offset),
    Source.CS.CS.initial_state p s ->
    Star (CS.sem p) s t s' ->
    Memory.Memory.load (CS.s_memory s') ptr =
    Some (Ptr (Permission.data, C, b, o)) ->
    exists Cmem : eqtype.Equality.sort Memory.ComponentMemory.t,
      CS.s_memory s' C = Some Cmem /\
      b < Memory.ComponentMemory.next_block Cmem
CS.load_component_prog_interface_addr
  : forall p : Source.program,
    Source.well_formed_program p ->
    Source.closed_program p ->
    ...
    Source.CS.CS.initial_state p s ->
    Star (CS.sem p) s t s' ->
    Memory.Memory.load (CS.s_memory s') ptr = Some v ->
    Pointer.component ptr
      \in domm (T:=nat_ordType) (S:=Component.interface)
...
CS.load_component_prog_interface
  : forall p : Source.program,
    ...
    Source.CS.CS.initial_state p s ->
    Star (CS.sem p) s t s' ->
    Memory.Memory.load (CS.s_memory s') ptr = Some (Ptr ptr') ->
    Pointer.component ptr'
      \in domm (T:=nat_ordType) (S:=Component.interface)
            (Source.prog_interface p)
CSInvariants.CSInvariants.load_Some_component_buffer
  : forall (p : Intermediate.program) (s : CS.state) 
      (t : list Events.event) (e : Events.event) (ptr : Pointer.t)
      (v : value),
    Intermediate.well_formed_program p ->
    Intermediate.closed_program p ->
    CSInvariants.CSInvariants.is_prefix s p (seq.rcons t e) ->
    Memory.Memory.load (mem_of_event e) ptr = Some v ->
    Pointer.component ptr
      \in domm (T:=nat_ordType) (S:=Component.interface)
...
CS.intermediate_well_formed_events
  : forall p : Intermediate.program,
    Intermediate.well_formed_program p ->
    Intermediate.closed_program p ->
    forall (st : state (CS.sem_inform p)) (t : Events.trace event_inform)
      (st' : state (CS.sem_inform p)),
    Star (CS.sem_inform p) st t st' ->
    seq.all
      (well_formed_event (Intermediate.prog_interface p)
         (Intermediate.prog_procedures p)) t
Definability.initialization_correct_component_memory
  : forall (C : Component.id)
      (mem mem' : eqtype.Equality.sort Memory.Memory.t),
    (forall (C' : Component.id) (b : Block.id) (offset : Block.offset),
     C <> C' ->
     Memory.Memory.load mem (Permission.data, C', b, offset) =
     Memory.Memory.load mem' (Permission.data, C', b, offset)) ->
    (forall C' : Component.id,
     C <> C' ->
     Definability.next_block mem C' = Definability.next_block mem' C') ->
    forall C' : Component.id, C <> C' -> mem C' = mem' C'
Definability.initialization_correct
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    forall (C : nat_ordType) (stk : Source.CS.CS.stack)
      (mem : eqtype.Equality.sort Memory.Memory.t) 
      (k : cont) (arg : value) (prefix : Events.trace event_inform)
      (e : event_inform),
    Definability.component_buffer intf C ->
    Definability.postcondition_steady_state e mem C \/
    Definability.postcondition_uninitialized prog_buffers prefix e mem C ->
    exists (mem' : eqtype.Equality.sort Memory.Memory.t) 
    (i : Z),
      star CS.kstep
        (prepare_global_env
           (Definability.program_of_trace intf prog_buffers t))
        [CState C, stk, mem, k, Definability.init_check prog_buffers C, arg]
        Events.E0 [CState C, stk, mem', k, E_val (Int i), arg] /\
      Definability.postcondition_steady_state e mem' C /\
      (forall offset : Z,
       offset <> Definability.INITFLAG_offset ->
       offset <> Definability.LOCALBUF_offset ->
       Memory.Memory.load mem (Permission.data, C, Block.local, offset) =
       Memory.Memory.load mem' (Permission.data, C, Block.local, offset)) /\
      (forall (C' : nat_ordType) (b : Block.id) (offset : Block.offset),
       C <> C' ->
       Memory.Memory.load mem (Permission.data, C', b, offset) =
      ...
      (forall (C' : nat_ordType) (b : Block.id) (offset : Block.offset),
       ...
       Memory.Memory.load mem' (Permission.data, C', b, offset))
Compiler.index : Type
Intermediate.in_unzip2
  : forall (X : eqtype.Equality.type) (x : eqtype.Equality.sort X)
      (xs : NMap (eqtype.Equality.sort X)),
    x \in seq.unzip2 xs -> exists n : nat_ordType, xs n = Some x
CS.genv_procedures_prog_procedures
  : forall (p : Intermediate.program) (cid : nat_ordType)
      (proc : option (NMap code)),
    Intermediate.well_formed_program p ->
    GlobalEnv.genv_procedures (globalenv (CS.sem_inform p)) cid = proc <->
    Intermediate.prog_procedures p cid = proc
GlobalEnv.genv_entrypoints_interface_some
  : forall (p p' : Intermediate.program) (C : Component.id)
      (P : Procedure.id) (b : Block.id),
    Intermediate.well_formed_program p ->
    Intermediate.well_formed_program p' ->
    Intermediate.prog_interface p = Intermediate.prog_interface p' ->
    Intermediate.EntryPoint.get C P
      (GlobalEnv.genv_entrypoints (GlobalEnv.prepare_global_env p)) = 
    Some b ->
    exists b' : Block.id,
      Intermediate.EntryPoint.get C P
        (GlobalEnv.genv_entrypoints (GlobalEnv.prepare_global_env p')) =
...
CS.eval_kstep_sound
  : forall (G : global_env) (st : Source.CS.CS.state)
      (t : Events.trace Events.event) (st' : Source.CS.CS.state),
    CS.eval_kstep G st = Some (t, st') -> CS.kstep G st t st'
Definability.definability_does_not_leak
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    Intermediate.closed_program p_interm ->
    Intermediate.prog_interface p_interm = intf ->
    CS.private_pointers_never_leak_S
      (Definability.program_of_trace intf prog_buffers t) 
      (uniform_shift 1)
Memory.component_memory_after_store_neq
  : forall (mem : eqtype.Equality.sort Memory.Memory.t) 
      (ptr : Pointer.t) (v : value)
      (mem' : eqtype.Equality.sort Memory.Memory.t) 
      (C : Component.id),
    Memory.Memory.store mem ptr v = Some mem' ->
    Pointer.component ptr <> C -> mem C = mem' C
Memory.component_memory_after_alloc_neq
  : forall (mem : eqtype.Equality.sort Memory.Memory.t) 
      (C : Component.id) (sz : nat)
      (mem' : eqtype.Equality.sort Memory.Memory.t) 
      (ptr : Pointer.t) (C' : Component.id),
    Memory.Memory.alloc mem C sz = Some (mem', ptr) ->
    C' <> C -> mem C' = mem' C'
...
Source.CS.CS.comes_from_initial_state_mem_domm
  : forall p : Source.program,
    ...
    Source.CS.CS.initial_state p s ->
    Star (CS.sem p) s t s' ->
    domm (T:=nat_ordType) (S:=eqtype.Equality.sort Memory.ComponentMemory.t)
      (CS.s_memory s') =
    domm (T:=nat_ordType) (S:=Component.interface) (Source.prog_interface p)
Classical_Prop.classic : forall P : Prop, P \/ ~ P
...
Memory.Memory.alloc_after_load
  : forall (mem : eqtype.Equality.sort Memory.Memory.t) 
      (P : Permission.id) (C : Component.id) (b : Block.id)
      (o : Block.offset) (v : value) (size : nat),
    Memory.Memory.load mem (P, C, b, o) = Some v ->
    exists (mem' : eqtype.Equality.sort Memory.Memory.t) 
    (b' : Block.id),
      b' <> b /\
      Memory.Memory.alloc mem C size =
      Some (mem', (Permission.data, C, b', 0%Z))
Definability.addr_shared_so_far_inv_2
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    Intermediate.closed_program p_interm ->
    Intermediate.prog_interface p_interm = intf ->
    forall (ret_val : value) (mem' : Memory.Memory.tt)
      (regs : Intermediate.Register.t) (C C' : Component.id)
      (s : stack_state) (prefix0 : list event_inform)
      (eprev ecur : event_inform) (ecur_noninf : Events.event),
    project_non_inform [ecur] = [ecur_noninf] ->
    Definability.well_formed_intermediate_prefix intf prog_buffers
      (seq.cat (seq.cat prefix0 [eprev]) [ecur]) ->
    forall
      mem0 mem mem1 mem1' mem2 mem3 mem4 mem5 mem6 mem7 mem8
       mem9 : Memory.Memory.tt,
    C = cur_comp s ->
    forall vcom : value,
    Memory.Memory.load mem0 (Permission.data, cur_comp s, Block.local, 5%Z) =
    Some vcom ->
    Memory.Memory.store mem
      (Permission.data, C, Block.local, Definability.EXTCALL_offset) 
      (Int 1) = Some mem1 ->
    Memory.Memory.store mem1 (Permission.data, C', Block.local, 5%Z) vcom =
    Some mem1' ->
    Memory.Memory.store mem1' (Permission.data, C', Block.local, 4%Z) Undef =
    Some mem2 ->
    Memory.Memory.store mem2 (Permission.data, C', Block.local, 6%Z) Undef =
    Some mem3 ->
    Memory.Memory.store mem3 (Permission.data, C', Block.local, 7%Z) Undef =
    Some mem4 ->
    Memory.Memory.store mem4 (Permission.data, C', Block.local, 8%Z) Undef =
    Some mem5 ->
    Memory.Memory.store mem5 (Permission.data, C', Block.local, 9%Z) Undef =
    Some mem6 ->
    Memory.Memory.store mem6 (Permission.data, C', Block.local, 10%Z) Undef =
    Some mem7 ->
    Memory.Memory.store mem7
      (Permission.data, C', Block.local, Definability.EXTCALL_offset) 
      (Int 0) = Some mem8 ->
    forall (Cb : Component.id) (b : Block.id) (addr' : addr_t),
    addr_shared_so_far (Cb, b)
      (seq.rcons (project_non_inform (seq.cat prefix0 [eprev])) ecur_noninf) ->
    Definability.postcondition_event_registers
      (ERetInform (cur_comp s) ret_val mem' regs C') mem9 ->
    (forall C0 : nat_ordType,
     Definability.component_buffer intf C0 ->
     C0 =
     Events.next_comp_of_event (ERetInform (cur_comp s) ret_val mem' regs C') ->
     Definability.postcondition_steady_state
       (ERetInform (cur_comp s) ret_val mem' regs C') mem9 C0) ->
    (forall C0 : nat_ordType,
     Definability.component_buffer intf C0 ->
     C0 <>
     ...
     Definability.postcondition_steady_state
       (ERetInform (cur_comp s) ret_val mem' regs C') mem9 C0 \/
     Definability.postcondition_uninitialized prog_buffers
       (seq.cat prefix0 [eprev]) ecur mem9 C0) ->
    Reachability.Reachable (mem_of_event (Events.ERet C vcom mem1 C'))
      (fset1 (T:=prod_ordType nat_ordType nat_ordType) addr') 
      (Cb, S b)
Definability.addr_shared_so_far_inv_1
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    forall C C' : Component.id,
    C <> C' ->
    forall (prefix0 : list event_inform) (eprev ecur : event_inform)
      (ecur_noninf : Events.event),
    project_non_inform [ecur] = [ecur_noninf] ->
    Definability.well_formed_intermediate_prefix intf prog_buffers
      (seq.cat (seq.cat prefix0 [eprev]) [ecur]) ->
    forall
      (mem0 mem mem1 mem1' mem2 mem3 mem4 mem5 mem6 mem7
       mem8 : Memory.Memory.tt) (vcom : value),
    Memory.Memory.load mem0 (Permission.data, C, Block.local, 5%Z) =
    Some vcom ->
    Memory.Memory.store mem
      (Permission.data, C, Block.local, Definability.EXTCALL_offset) 
      (Int 1) = Some mem1 ->
    ...
    forall (Cb : Component.id) (b : Block.id),
    Reachability.Reachable mem' (addr_of_value ret_val) (Cb, b) ->
    Definability.postcondition_event_registers ecur mem8 ->
    addr_shared_so_far (Cb, b)
      (seq.rcons (project_non_inform (seq.cat prefix0 [eprev])) ecur_noninf) ->
    (forall C0 : nat_ordType,
     Definability.component_buffer intf C0 ->
     C0 = Events.next_comp_of_event ecur ->
     Definability.postcondition_steady_state ecur mem8 C0) ->
    (forall C0 : nat_ordType,
     Definability.component_buffer intf C0 ->
     C0 <> Events.next_comp_of_event ecur ->
     Definability.postcondition_steady_state ecur mem8 C0 \/
     Definability.postcondition_uninitialized prog_buffers (...) ecur mem8 C0) ->
    C = Events.cur_comp_of_event ecur_noninf ->
    C' = Events.next_comp_of_event ecur ->
    mem' = mem_of_event_inform ecur ->
    ret_val = arg_of_event ecur_noninf ->
    Reachability.Reachable mem1 (addr_of_value vcom) (Cb, S b)
Definability.addr_shared_so_far_ERet_Hsharedsrc
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    Intermediate.closed_program p_interm ->
    Intermediate.prog_interface p_interm = intf ->
    forall (ret_val : value) (mem' : eqtype.Equality.sort Memory.Memory.t)
      (regs : Intermediate.Register.t) (C' : Component.id)
      (prefix suffix : list event_inform) (s : stack_state),
    Definability.well_formed_intermediate_prefix intf prog_buffers
      (seq.cat prefix [ERetInform (cur_comp s) ret_val mem' regs C']) ->
    forall (prefix' : Events.trace Events.event) (mem0 : Memory.Memory.tt)
      (mem mem1 : eqtype.Equality.sort Memory.Memory.t),
    Memory.Memory.store mem
      (Permission.data, cur_comp s, Block.local, Definability.EXTCALL_offset)
      (Int 1) = Some mem1 ->
    forall vcom : value,
    Memory.Memory.load mem0
      (Permission.data, cur_comp s, Block.local,
      Definability.reg_offset E_R_COM) = Some vcom ->
    forall mem1' : eqtype.Equality.sort Memory.Memory.t,
    Memory.Memory.store mem1
      (Permission.data, C', Block.local, Definability.reg_offset E_R_COM)
      vcom = Some mem1' ->
    forall mem2 : eqtype.Equality.sort Memory.Memory.t,
    Memory.Memory.store mem1'
      (Permission.data, C', Block.local, Definability.reg_offset E_R_ONE)
      Undef = Some mem2 ->
    forall mem3 : eqtype.Equality.sort Memory.Memory.t,
    Memory.Memory.store mem2
      (Permission.data, C', Block.local, Definability.reg_offset E_R_AUX1)
      Undef = Some mem3 ->
    forall mem4 : eqtype.Equality.sort Memory.Memory.t,
    Memory.Memory.store mem3
      (Permission.data, C', Block.local, Definability.reg_offset E_R_AUX2)
      Undef = Some mem4 ->
    forall mem5 : eqtype.Equality.sort Memory.Memory.t,
    Memory.Memory.store mem4
      (Permission.data, C', Block.local, Definability.reg_offset E_R_RA)
      Undef = Some mem5 ->
    forall mem6 : eqtype.Equality.sort Memory.Memory.t,
    Memory.Memory.store mem5
      (Permission.data, C', Block.local, Definability.reg_offset E_R_SP)
      Undef = Some mem6 ->
    forall mem7 : eqtype.Equality.sort Memory.Memory.t,
    Memory.Memory.store mem6
      (Permission.data, C', Block.local, Definability.reg_offset E_R_ARG)
      Undef = Some mem7 ->
    forall mem8 : eqtype.Equality.sort Memory.Memory.t,
    Memory.Memory.store mem7
      (Permission.data, C', Block.local, Definability.EXTCALL_offset) 
      (Int 0) = Some mem8 ->
    forall (s' : stack_state) (cs' : Source.CS.CS.state),
    Definability.well_formed_state_r intf prog_buffers t T s'
      (seq.cat prefix [ERetInform (cur_comp s) ret_val mem' regs C']) suffix
      cs' ->
    forall (Cb : Component.id) (b : Block.id),
    addr_shared_so_far (Cb, b)
      (seq.rcons prefix' (Events.ERet ... vcom mem1 C')) ->
    exists addr : addr_t,
      sigma_shifting_wrap_bid_in_addr (...) addr = Some (Cb, b) /\
      event_renames_event_at_shared_addr all_zeros_shift 
        (...) addr (...) (...) /\ addr_shared_so_far addr (...)
Definability.addr_shared_so_far_ECall_Hshared_src
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    Intermediate.closed_program p_interm ->
    Intermediate.prog_interface p_interm = intf ->
    forall (P' : Procedure.id) (new_arg : value)
      (mem' : eqtype.Equality.sort Memory.Memory.t)
      (regs : Intermediate.Register.t) (C' : Component.id)
      (prefix suffix : list event_inform) (s : stack_state),
    Definability.well_formed_intermediate_prefix intf prog_buffers
      (seq.cat prefix [ECallInform (cur_comp s) P' new_arg mem' regs C']) ->
    forall (prefix' : Events.trace Events.event) (stk : Source.CS.CS.stack)
      (mem0 : Memory.Memory.tt) (arg : value) (P : Procedure.id)
      (mem : eqtype.Equality.sort Memory.Memory.t) 
      (vcom : value),
    Memory.Memory.load mem0
      (Permission.data, cur_comp s, Block.local,
    ...
    (forall offset : Z,
     offset <> Definability.INITFLAG_offset ->
     offset <> Definability.LOCALBUF_offset ->
     Memory.Memory.load mem1 (Permission.data, C', Block.local, offset) =
     Memory.Memory.load mem2 (Permission.data, C', Block.local, offset)) ->
    forall mem3 : eqtype.Equality.sort Memory.Memory.t,
    Memory.Memory.store mem2
      (Permission.data, C', Block.local, Definability.reg_offset E_R_ONE)
      Undef = Some mem3 ->
    ...
    Memory.Memory.store mem7
      (Permission.data, C', Block.local, Definability.reg_offset E_R_ARG)
      Undef = Some mem8 ->
    forall mem9 : eqtype.Equality.sort Memory.Memory.t,
    Memory.Memory.store mem8
      (Permission.data, C', Block.local, Definability.reg_offset E_R_COM)
      vcom = Some mem9 ->
    forall mem10 : eqtype.Equality.sort Memory.Memory.t,
    Memory.Memory.store mem9 (Permission.data, C', Block.local, 1%Z) (Int 0) =
    Some mem10 ->
    Definability.well_formed_state_r intf prog_buffers t T
      {| cur_comp := C'; callers := cur_comp s :: callers s |}
      (seq.cat prefix [ECallInform (cur_comp s) P' new_arg mem' regs C'])
      suffix
      [CState C', {|
                  ...
                  S.CS.f_arg := arg;
                  S.CS.f_cont := Kassign1 (...) (...) |} :: stk, mem10, Kstop, 
      Definability.expr_of_trace C' P' (Definability.comp_subtrace C' t), vcom] ->
    forall (Cb : Component.id) (b : Block.id),
    addr_shared_so_far (Cb, b) (seq.rcons prefix' (...)) ->
    exists addr : addr_t,
      sigma_shifting_wrap_bid_in_addr ... addr = Some ... /\
      event_renames_event_at_shared_addr all_zeros_shift ... addr ... ... /\
      addr_shared_so_far addr ...
Definability.addr_shared_so_far_ECall_Hshared_interm
  : forall intf : Program.interface,
    closed_interface intf ->
    intf Component.main ->
    forall prog_buffers : NMap (nat + list value),
    ...
    Intermediate.prog_interface p_interm = intf ->
    forall (P P' : Procedure.id) (C C' : Component.id) 
      (s : stack_state) (prefix : list event_inform)
      (prefix' : list Events.event) (new_arg : value)
      (mem' : eqtype.Equality.sort Memory.Memory.t)
      (regs : Intermediate.Register.t) (suffix : Events.trace event_inform)
      (arg : value) (stk : list CS.frame) (mem1 : Memory.Memory.tt)
      (mem10 : eqtype.Equality.sort Memory.Memory.t) 
      (vcom : value),
    Definability.well_formed_state_r intf prog_buffers t T
      {| cur_comp := C'; callers := C :: callers s |}
      (seq.cat prefix [ECallInform (cur_comp s) P' new_arg mem' regs C'])
      ...
      [CState C', {|
                  S.CS.f_component := C;
                  S.CS.f_arg := arg;
                  S.CS.f_cont := Kassign1 (Definability.loc_of_reg E_R_COM)
                                   (Kseq
                                      (E_seq Definability.invalidate_metadata
                                         (E_seq
                                            (E_assign Definability.EXTCALL
                                               (E_val (Int 0)))
                                            (E_call C P (E_val (Int 0)))))
                                      Kstop) |} :: stk, mem10, Kstop, 
      Definability.expr_of_trace C' P' (Definability.comp_subtrace C' t), vcom] ->
    forall (Cb : Component.id) (b : Block.id),
    addr_shared_so_far (Cb, b)
      (seq.rcons (project_non_inform prefix)
         (Events.ECall (cur_comp s) P' new_arg mem' C')) ->
    addr_shared_so_far (Cb, S b)
      (seq.rcons prefix' (Events.ECall C P' vcom mem1 C'))
