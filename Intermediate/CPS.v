...
Module CPS.
  ...
  | Return: forall mem C pc pc' d' regs d C',
      ...
      G |-CPS (C,d,mem,regs,pc) =>[t] (C',d',mem,regs,pc')

  where "G |-CPS s1 '=>[' t ']' s2" := (step G s1 t s2).
  ...
  Lemma epsilon_step_weakening:
    forall Is E C d1 mem mem' cmem cmem' regs regs' pc pc',
      ...
      M.MapsTo C cmem  mem ->
      M.MapsTo C cmem' mem' ->
      G |-CPS (C,d1,mem,regs,pc) =>[E0] (C,d1,mem',regs',pc') ->
      forall E' d2 wmem,
        let G' := mkGlobalEnv Is E' in
        M.MapsTo C cmem wmem ->
        (G' |-CPS (C,d2,wmem,regs,pc) =>[E0] (C,d2,wmem,regs',pc'))
        \/
        (forall wmem',
