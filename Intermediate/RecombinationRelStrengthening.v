...
Section ThreewayMultisem1.
  ...
  Lemma Reachable_memrel_Reachable ev mem0 regs t1 s1'mem s1'reg addrl:
    good_trace_extensional (left_addr_good_for_shifting n)
                           (rcons t1 ev)
    ->
    arg_of_event ev = (Register.get R_COM regs)
    ->
    mem_of_event ev = mem0
    ->
    (
      shift_value_option
        n
    ...
    exists addrl',
      ...
      Reachable s1'mem (addr_of_value (Register.get R_COM s1'reg))
                addrl'.
  Proof.
    intros Hgoodrconst1 arg_ev mem_ev Hrel_R_COM mem0_s1'mem Hreach.
    ...
    - inversion HR_COM as [Hrewr]. simpl in *. rewrite epermv.
      ...
      + assert (addr_shared_so_far
               ...
               ) as Hshr.
        {
          ...
          rewrite arg_ev; simpl. rewrite epermv; auto.
        }
        ...
        assert (exists offl offset,
                   Memory.load (mem_of_event ev) (Permission.data, cid, bid, offset) =
    ...
    - (** Here, derive a contradiction to ebidv. *)
             ...
             ) as [? contra].
      {
        ...
        rewrite arg_ev; simpl. rewrite epermv; constructor; by rewrite in_fset1.
      }
      by rewrite contra in ebidv.
  Qed.
  
  Corollary Reachable_memrel_Reachable_corollary:
    forall ev mem0 regs t1 s1'mem s1'reg cidl bidl bidl',
    good_trace_extensional (left_addr_good_for_shifting n)
                           (rcons t1 ev)
    ->
    arg_of_event ev = (Register.get R_COM regs)
  ...
  Proof.
    intros ? ? ? ? ? ? ? ? ? H1 Harg Hmem H2 H3 H4.
    specialize (Reachable_memrel_Reachable H1 Harg Hmem H2 H3 H4)
  ...
  Lemma Reachable_memrel_Reachable2 ev mem0 regs t1 s1'mem s1'reg addrl':
    good_trace_extensional (left_addr_good_for_shifting n)
                           (rcons t1 ev)
    ->
    arg_of_event ev = (Register.get R_COM regs)
    ...
    exists addrl,
      ...
      Reachable mem0 (addr_of_value (Register.get R_COM regs))
                addrl.
  Proof.
    intros Hgoodrconst1 Harg Hmem Hrel_R_COM mem0_s1'mem Hreach.
    unfold event_renames_event_at_shared_addr,
    mem_of_part_not_executing_rel_original_and_recombined_at_border,
    mem_of_part_executing_rel_original_and_recombined,
    mem_of_part_not_executing_rel_original_and_recombined_at_internal,
    ...
    - destruct (permv_regs =? Permission.data) eqn:epermv_regs.
      + destruct (sigma_shifting_lefttoright_option
        ...
        * destruct IH as [[cidl bidl] [IH1 IH2]].
        ...
        {
          eapply reachable_from_args_is_shared; rewrite Harg; simpl in *;
            rewrite epermv; auto.
        }
        specialize (shared_good _ Hshr); simpl in *.
        destruct mem0_s1'mem as [_ [mem0s1'mem _]].
        ...
        destruct (permv' =? Permission.data) eqn:epermv'.
          --  destruct (sigma_shifting_lefttoright_option
              ...
              eexists; split; eauto.
              unfold Memory.load in G. simpl in *.
              destruct ((mem_of_event ev) cidl') eqn:emem0; try discriminate.
    ...
    - destruct (permv_regs =? Permission.data) eqn:epermv_regs; try discriminate.
      ...
      {
        econstructor; eauto; rewrite Harg; simpl; rewrite epermv;
  ...
  Corollary Reachable_memrel_Reachable2_corollary
            ev mem0 regs t1 s1'mem s1'reg addrl' addrl:
    good_trace_extensional (left_addr_good_for_shifting n)
                           (rcons t1 ev)
    ->
    arg_of_event ev = (Register.get R_COM regs)
  ...
  Proof.
    intros H1 Harg Hmem H2 H3 H4.
    specialize (Reachable_memrel_Reachable2 H1 Harg Hmem H2 H3 H4)
      as [[cidl' bidl'2] [ebidl'2 G]].
    unfold sigma_shifting_wrap_bid_in_addr, sigma_shifting_lefttoright_addr_bid in *.
    intros rewr; rewrite rewr in ebidl'2; inversion ebidl'2; by subst.
  Qed.
  ...
  Lemma shared_Reachable_memrel_shared_Reachable
        ev mem0 regs t1 t1' s1'mem addrl addr_shr:
  ...
  Proof.
    intros t1good Harg Hmem t1t1' mem0s1'mem Hshr Hreach.
    ...
    ****************)
    ...
    - destruct addr_shr as [shrcid shrbid]. simpl in *.
      ...
      + destruct (sigma_shifting_lefttoright_option
        ...
        unfold Memory.load in *. simpl in *.
        destruct ((mem_of_event ev) cid'') eqn:emem0; try discriminate.
        eapply Reachable_step; first eassumption; first eassumption.
        rewrite Extra.In_in ComponentMemory.load_block_load.
        do 2 eexists; eassumption.
        ...
        * apply rcons_inj in Hr5. apply rcons_inj in Hr6. inversion Hr5.
                 ...
                 ).
          {
            ...
            specialize (G
                          ...
                          _ _ _ _ _ _
                          Hgood2 Logic.eq_refl Logic.eq_refl
                          Hrel_R_COM mem0_s1'mem Hreach)
              as [[? ?] [rewr ?]].
            destruct (sigma_shifting_lefttoright_option
                        (if cidl \in domm (prog_interface p) then n cidl else n'' cidl) 
        ...
        * apply rcons_inj in Hr5. apply rcons_inj in Hr6. inversion Hr5.
          ...
          assert (exists ptr' stk',
                     ...
                     stack_of_program_part_rel_stack_of_recombined
                       ...
                       stk'
                     /\
                     if Pointer.component pc' \in domm (prog_interface p)
                     then pc' = ptr'
                     else Pointer.component pc' = Pointer.component ptr'
                 ) as [ptr' [stk' [Hsubst [Hstk'_gps' Hptr'_pc']]]]; subst.
          {
            inversion Hstack_s_s'; subst.
            exists ptr_recombined; exists stk_recombined.
            by intuition.
          }
            ...
            ++ eapply (@CS.Return _ _ _ _ _ _ _ _);
               ...
               ** simpl in *; rewrite Hpc.
                  destruct (Pointer.component pc' \in domm (prog_interface p)) eqn:e;
                    rewrite e in Hptr'_pc'; by rewrite <- Hptr'_pc'.
               ** (** here, use the register relation (Hrel_R_COM) **)
                 by intuition.
            ++ simpl in *.
               destruct (Pointer.component pc' \in domm (prog_interface p)) eqn:e;
          ...
          assert (Hisprefix1: CSInvariants.is_prefix
                                ...
                                (rcons t1' (ERet
                                              (Pointer.component pc)
                                              (Register.get R_COM s1'reg) s1'mem
                                              (Pointer.component pc')))).
          {
            unfold CSInvariants.is_prefix.
            eapply star_right; try eassumption.
          ...
          assert (Hcomps2'pc: Pointer.component s2'pc = Pointer.component pc').
          {
            inversion Hstep12'; subst.
            destruct t; try discriminate.
            match goal with
            ...
            simpl in *.
            by destruct (Pointer.component pc' \in domm (prog_interface p)) eqn:e;
              rewrite e in Hptr'_pc'; rewrite Hptr'_pc'.
          }
          ...
          -- apply mergeable_border_states_p_executing;
             ...
             ++ simpl.
                ...
                by rewrite C'0_prog in Hptr'_pc'.
          ...
          -- apply mergeable_border_states_c'_executing.
             ...
             ++ simpl.
                ...
                match goal with | rewr: _ pc' = _ s2''pc |- _ =>
                                  rewrite <- rewr in Hptr'_s2''pc
                end.

                by rewrite C'0_ctx in Hptr'_s2''pc.
