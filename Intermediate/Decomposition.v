...
Section Decomposition.
  ...
  Let split := map fst (PMap.elements (prog_interface p)).
  ...
  | ProgramControl:
      forall gps pgps mem pmem regs pc,
        ...
        (forall C, PMap.In C (prog_interface (program_link p c mainC mainP)) <->
              PMap.In C mem) ->
        (* pmem contains exactly all program components memories of mem *)
        (forall C, PS.is_program_component G' C ->
              (forall Cmem, PMap.MapsTo C Cmem mem <-> PMap.MapsTo C Cmem pmem)) ->
  ...
  Proof.
    ...
    destruct (PMapExtra.F.In_dec (prog_interface p) mainC) as [Hprg|Hctx].
    (* program has control case *)
    - exists (PS.PC (PS.to_partial_stack s split,
                PMapExtra.filter (fun C _ => PMap.mem C (prog_interface p)) mem,
                regs, pc)).
      split.
        ...
        * split.
          ** intro C. split.
             *** intro HCprog.
                 ...
                 apply PMapExtra.diff_in_iff in HCprog. destruct HCprog.
                 apply PMapExtra.update_in_iff in H. destruct H.
                 **** specialize (Hmem1 C).
                      assert (HCinPC: PMap.In C (prog_interface
                                                  (program_link p c mainC mainP))). {
                        unfold program_link. simpl.
                        apply PMapExtra.update_in_iff. left. auto.
                      ...
                      ***** split.
                      ****** auto.
                      ****** apply PMapExtra.F.mem_in_iff. auto.
                 **** contradiction.
             *** intro.
                 destruct H.
                 apply PMapExtra.filter_iff in H.
                 **** destruct H. apply PMapExtra.F.mem_in_iff in H0.
                      unfold PS.is_program_component, G', init_genv.
                      destruct (init_all (PS.partialize (program_link p c mainC mainP)
                                                        (prog_interface c))).
                      destruct p0. simpl.
                      apply PMapExtra.diff_in_iff. split.
                      ***** apply PMapExtra.update_in_iff. left. auto.
                      ***** destruct linkability as [? [? []]].
                      unfold PMapExtra.Disjoint in H3.
             ...
             *** split.
                 ...
                 **** split.
                      ...
                      ****** unfold linkable_programs in linkability.
                      ...
                      unfold PMapExtra.Disjoint in Hdisjoint.
      ...
      + apply ProgramControl.
        * simpl in *. rewrite Hpc_comp.
          ...
          ** apply PMapExtra.update_in_iff. left. auto.
          ** destruct linkability as [? [? []]].
             unfold PMapExtra.Disjoint in H1.
        ...
        * intros C HCprog Cmem. split.
          ** intro. apply PMapExtra.filter_iff.
             *** unfold Morphisms.Proper, Morphisms.respectful.
                 intros. subst. reflexivity.
             *** split. auto.
                 unfold PS.is_program_component in HCprog.
                 ...
                 destruct p0. simpl in HCprog.
                 apply PMapExtra.diff_in_iff in HCprog. destruct HCprog.
                 apply PMapExtra.update_in_iff in H0. destruct H0.
                 **** apply PMapExtra.F.mem_in_iff. auto.
                 **** contradiction.
          ** intro.
             unfold PS.is_program_component in HCprog.
        ...
        * split.
          ** intro C. split.
             *** intro HCinprog.
                 ...
                 apply PMapExtra.diff_in_iff in HCinprog.
                 destruct HCinprog as [H1 H2].
                 specialize (Hmem1 C).
                 unfold program_link in Hmem1. simpl in Hmem1.
                 assert (H1' := H1).
                 apply Hmem1 in H1. destruct H1.
                 exists x. apply PMapExtra.filter_iff.
                 **** unfold Morphisms.Proper, Morphisms.respectful.
                      intros. subst. reflexivity.
                 **** split.
                      ***** apply H.
                      ***** apply PMapExtra.F.mem_in_iff. 
                      apply PMapExtra.update_in_iff in H1'.
             ...
             *** intro HCinmem.
                 ...
                 destruct p0. simpl.
                 apply PMapExtra.diff_in_iff. split.
                 **** destruct HCinmem. apply PMapExtra.filter_iff in H.
                      ***** destruct H. apply PMapExtra.F.mem_in_iff in H0.
                      apply PMapExtra.update_in_iff. left. auto.
                      ***** unfold Morphisms.Proper, Morphisms.respectful.
                      intros. subst. reflexivity.
                 ...
                 **** destruct linkability as [? [? []]].
                      ...
                      apply H1. split.
                      ***** destruct HCinmem. apply PMapExtra.filter_iff in H4.
                      destruct H4. apply PMapExtra.F.mem_in_iff in H5. auto.
                      ****** unfold Morphisms.Proper, Morphisms.respectful.
                      intros. subst. reflexivity.
                      ***** auto.
          ** split.
        ...
        * intros. split.
          ** intro.
             ...
             *** split. auto.
                 ...
                 apply PMapExtra.diff_in_iff in H. destruct H.
                 apply PMapExtra.update_in_iff in H. destruct H; try contradiction.
                 auto.
          ** intro. apply PMapExtra.filter_iff in H0.
             *** destruct H0. auto.
             *** unfold Morphisms.Proper, Morphisms.respectful.
                 intros. subst. reflexivity.
     *)
...
(*
    ...
    - destruct (PMapExtra.F.In_dec (genv_entrypoints G) C') as [Htarget|Htarget].
      (* internal call *)
      + eexists. split.
      ...
      + eexists. split.
        ...
        * apply ContextControl_Normal.
          ...
          ** admit.
          ** auto.
    - destruct (PMapExtra.F.In_dec (genv_entrypoints G) (Pointer.component pc'))
                 as [Htarget|Htarget].
      (* internal return *)
      + eexists. split.
