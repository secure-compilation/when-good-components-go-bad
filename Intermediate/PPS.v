...
Module PPS.
  ...
  Definition to_partial_stack (s : CPS.stack) (pc : list Component.id) :=
  ...
  Reserved Notation "G |-PPS s1 '=>[' t ']' s2" (at level 40).
  ...
  | Program_Epsilon:
      forall E C s ps
        ...
        let G' := CPS.mkGlobalEnv (genv_interfaces G) E in
        CPS.step G' (C,s,wmem,regs,pc) E0 (C,s,wmem',regs',pc') ->
        M.MapsTo C cmem mem ->
        M.MapsTo C cmem' mem' ->
        G |-PPS (PC (C,ps,mem,regs,pc)) =>[E0] (PC (C,ps,mem',regs',pc'))
  ...
  | Program_Internal_Return:
      forall mem C C' pc pc' d d' regs,
        ...
        G |-PPS (PC (C,d,mem,regs,pc)) =>[t] (PC (C',d',mem,regs,pc'))
  ...
  | Program_External_Call:
      forall mem C pc regs d d' C' P,
        ...
        d' = (C,Some (pc+1)) :: d ->
        let t := [ECall C P (Register.get Register.R_COM regs) C'] in
        G |-PPS (PC (C,d,mem,regs,pc)) =>[t] (CC (C',d',mem) Normal)
  ...
  | Context_Epsilon:
      forall mem C d,
        G |-PPS (CC (C,d,mem) Normal) =>[E0] (CC (C,d,mem) Normal)

  | Context_GoesWrong:
      forall mem C d,
        G |-PPS (CC (C,d,mem) Normal) =>[E0] (CC (C,d,mem) WentWrong)
  ...
  | Context_Internal_Call:
      forall mem C d d' C' P call_arg,
        ...
        G |-PPS (CC (C,d,mem) Normal) =>[t] (CC (C',d',mem) Normal)
  ...
  | Context_External_Return:
      forall mem C C' pc' d d' regs,
        ...
        is_program_component C' (genv_split G) ->
        let t := [ERet C (Register.get Register.R_COM regs) C'] in
        G |-PPS (CC (C,d,mem) Normal) =>[t] (PC (C',d',mem,regs,pc'))

  where "G |-PPS s1 '=>[' t ']' s2" := (step G s1 t s2).
  ...
  Lemma entrypoints_exist_wrt_split split :
    forall p CI C,
      In CI (CPS.prog_interface p) ->
      Component.name CI = C ->
      In C split ->
      M.In C (CPS.prog_entrypoints p) /\
      exists addrs, M.MapsTo C addrs (CPS.prog_entrypoints p).
  Proof.
    intros p CI C.
    intros HCI_in_I HCI_name_is_C HC_in_split.
    apply (CPS.prog_entrypoints_wf p) with CI; assumption.
  Qed.

  Definition apply_split (p : CPS.program) split splitWF :=
    {| prog_interface := CPS.prog_interface p;
       prog_memory := CPS.prog_memory p;
       prog_entrypoints := CPS.prog_entrypoints p;
       prog_split := split;
       prog_entrypoints_wf :=
